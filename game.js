...
const SAVE_KEY = "proyecto_ultra_v4_1";
...
const skillInfo = {
 swords:{name:"Espadas",desc:"Dominio de espadas de una mano. Mejora precisión y daño físico.",how:"Golpea enemigos válidos usando una espada.",unlock:"60: Golpe poderoso · 80: Ignorar armadura"},
 unarmed:{name:"Combate sin armas",desc:"Técnicas de puños, patadas y control corporal.",how:"Golpea enemigos sin llevar un arma principal.",unlock:"60: Golpe aturdidor · 80: Llave de combate"},
 fencing:{name:"Esgrima",desc:"Uso veloz y preciso de dagas y kris.",how:"Golpea enemigos válidos usando dagas o kris.",unlock:"60: Estocada precisa · 80: Danza de cuchillas"},
 archery:{name:"Arquería",desc:"Uso de arcos normales y compuestos.",how:"Dispara flechas contra enemigos válidos.",unlock:"60: Disparo perforante · 80: Lluvia de flechas"},
 shieldMastery:{name:"Dominio del escudo",desc:"Mejora la probabilidad de bloquear golpes al usar escudo.",how:"Recibe ataques llevando escudo y realiza bloqueos exitosos.",unlock:"60: Guardia firme · 80: Contraataque"},
...
const skills = {};
Object.keys(skillInfo).forEach(k => skills[k] = {value:k==="swords"?35:k==="unarmed"?20:k==="fencing"?18:k==="archery"?16:k==="shieldMastery"?15:k==="tactics"?28:k==="healing"?22:k==="magery"?24:k==="tailoring"?12:k==="mining"?10:k==="blacksmithing"?8:18,progress:0});
...
function blocked(x,y){
 ...
 for(const b of buildings){
  if(x>b.x&&x<b.x+b.w&&y>b.y&&y<b.y+b.h){
   ...
   const south=Math.abs(x-cx)<1.25&&y>b.y+b.h-1.35;
   const north=Math.abs(x-cx)<1.25&&y<b.y+1.35;
   const west=Math.abs(y-cy)<1.25&&x<b.x+1.35;
   const east=Math.abs(y-cy)<1.25&&x>b.x+b.w-1.35;
   if(!(south||north||west||east))return true
  }
 }
...
const shops = {
 blacksmith:{name:"Herrería de Brom",items:["pickaxe","ironSword","dagger","kriss","shortBow","longBow","arrow","buckler","ironShield","ironHelm","ironChest","potion"]},
...
function attack(){
 ...
 const e=nearestEnemy(1.8);if(!e)return speak("No hay un enemigo al alcance.");
 const st=equipStats(),min=st.damageMin+skills.tactics.value*.08+player.attributes.strength*.04,max=st.damageMax+skills.tactics.value*.13+player.attributes.strength*.07;
 player.attackCd=skill==="fencing"?.42:skill==="unarmed"?.48:.62;player.attackAnim=1;
 let dmg=Math.round(rand(min,max)*(1+buffValue("damagePercent")/100));if(player.counterReady>0){dmg=Math.round(dmg*1.4);player.counterReady=0}
 setTimeout(()=>{if(e.alive&&dist(e,player)<2.1)damageEnemy(e,dmg)},150);
 gain(skill,.55,e.level);gain("tactics",.3,e.level);gainAttribute(skill==="fencing"?"dexterity":"strength",.16)
}
function magic(){
 if(!battleMode)return speak("Activa el modo batalla primero.");
 ...
 projectiles.push({kind:"fireball",x:player.x,y:player.y,z:20,vx:dx,vy:dy,speed:10,range:21,traveled:0,color:"#ff8c54",life:4,damage:Math.round(18+skills.magery.value*.35),owner:"player"});
...
function damagePlayer(n){
 ...
 const def=equipStats().defense+skills.defense.value*.18;n=Math.max(1,Math.round(n-def*.18));player.health-=n;player.hurtAnim=1;floatText(player.x,player.y,"-"+n,"#ffb0a5");
