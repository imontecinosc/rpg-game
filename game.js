...
function nearbyNpc(){
 const functional=npcs
  .filter(n=>n.role!=="ambient"&&distance(n,player)<2.25)
  .sort((a,b)=>distance(a,player)-distance(b,player));
 if(functional.length)return functional[0];
 return npcs
  .filter(n=>distance(n,player)<1.45)
  .sort((a,b)=>distance(a,player)-distance(b,player))[0]||null
}function nearbyCorpse(){return corpses.find(c=>distance(c,player)<1.4&&c.enemy.loot.length)}
...
function hint(){
 const npc=nearbyNpc();
 const t=npc?(shops[npc.role]?"Comerciar con "+npc.name:npc.role==="bank"?"Abrir banco con "+npc.name:npc.role.endsWith("Faction")?"Hablar con "+npc.name:"Hablar con "+npc.name):nearbyCorpse()?"Revisar botín":"";
 $("interactionHint").classList.toggle("hidden",!t);
 $("interactionHint").textContent=t
}
...
function openNpcDialogue(npc){
 ...
 const shop=shops[npc.role];if(shop){add("Comprar",()=>openShop(npc.role,"buy"));add("Vender",()=>openShop(npc.role,"sell"));add("Preguntar por rumores",()=>{$("dialogueText").textContent=choice(["Los bandidos se reúnen al este.","De noche aparecen espectros en el cementerio.","Algunas bestias dejan pieles útiles para Tailoring."])})}
...
// ==================== SPRITE DEL JUGADOR: IDLE ====================
const PLAYER_IDLE_FRAME_WIDTH = 192;
const PLAYER_IDLE_FRAME_HEIGHT = 256;
const PLAYER_IDLE_FRAMES = 2;
const PLAYER_IDLE_DIRECTIONS = 8;

const playerIdleSprite = new Image();
playerIdleSprite.src = "assets/characters/player/idle.png";
let playerIdleSpriteReady = false;

playerIdleSprite.addEventListener("load", () => {
  playerIdleSpriteReady = true;
});

playerIdleSprite.addEventListener("error", () => {
  console.warn("No se pudo cargar assets/characters/player/idle.png");
});

function getPlayerSpriteDirectionRow() {
  // Convertimos la dirección del mundo en dirección visible en pantalla.
  const screenX = player.dirX - player.dirY;
  const screenY = player.dirX + player.dirY;

  if (Math.abs(screenX) < 0.001 && Math.abs(screenY) < 0.001) {
    return player.lastSpriteDirection ?? 0;
  }
  ...
  // Filas de la hoja:
  // 0 Sur, 1 Suroeste, 2 Oeste, 3 Noroeste,
  // 4 Norte, 5 Noreste, 6 Este, 7 Sureste.
  const rowsByOctant = {
    0: 6,    // Este
    1: 7,    // Sureste
    2: 0,    // Sur
  ...
  const row = rowsByOctant[octant] ?? 0;
  player.lastSpriteDirection = row;
  return row;
}

function drawPlayerIdleSprite() {
  const p = screen(player.x, player.y);
  const directionRow = getPlayerSpriteDirectionRow();

  // Dos frames de respiración lenta.
  const frame = Math.floor(performance.now() / 650) % PLAYER_IDLE_FRAMES;

  const sourceX = frame * PLAYER_IDLE_FRAME_WIDTH;
  const sourceY = directionRow * PLAYER_IDLE_FRAME_HEIGHT;

  // Tamaño visible dentro del juego.
  const drawWidth = 96;
  const drawHeight = 128;

  // El punto del personaje está situado en sus pies.
  const drawX = p.x - drawWidth / 2;
  const drawY = p.y - drawHeight + 8;
  ...
  if (player.vitalShield > 0) {
    ctx.strokeStyle = "#9ce8b6aa";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.ellipse(p.x, p.y - 48, 30, 48, 0, 0, Math.PI * 2);
    ctx.stroke();
  }
...
// Conservamos el dibujado Canvas para NPC y enemigos.
// Solo reemplazamos el dibujo del personaje principal.
const humanoidCanvasFallback = humanoid;
humanoid = function(entity, color, enemy = false) {
  if (entity === player && playerIdleSpriteReady) {
    drawPlayerIdleSprite();
    return;
  }

  humanoidCanvasFallback(entity, color, enemy);
};
...
function updateKeyboardStick() {
  ...
  if (keyboardMovement.left) horizontal -= 1;
  if (keyboardMovement.right) horizontal += 1;
  if (keyboardMovement.up) vertical -= 1;
  if (keyboardMovement.down) vertical += 1;
...
window.addEventListener("keydown", event => {
  ...
  switch (key) {
    case "w":
    case "arrowup":
      keyboardMovement.up = true;
      break;
    case "s":
    case "arrowdown":
      keyboardMovement.down = true;
      break;
    case "a":
    case "arrowleft":
    ...
    case "f":
      ...
      break;
    case "q":
      if (!event.repeat) magic();
      break;
    case "e":
      if (!event.repeat) heal();
      break;
    case "r":
      if (!event.repeat) potion();
      break;
    case "1":
      if (!event.repeat && player.equippedSpells[0]) castSpell(player.equippedSpells[0]);
      break;
    case "2":
      if (!event.repeat && player.equippedSpells[1]) castSpell(player.equippedSpells[1]);
      break;
    case "3":
      if (!event.repeat && player.equippedSpells[2]) castSpell(player.equippedSpells[2]);
      break;
    case "4":
      if (!event.repeat && player.equippedSpells[3]) castSpell(player.equippedSpells[3]);
      break;
    case "z":
      if (!event.repeat) useSwordSpecial(1);
      break;
    case "x":
      if (!event.repeat) useSwordSpecial(2);
      break;
    case "i":
      if (!event.repeat) $("inventoryButton").click();
      break;
    case "k":
      if (!event.repeat) $("skillsButton").click();
      break;
    case "m":
      if (!event.repeat) $("mapButton").click();
      break;
    case "b":
      if (!event.repeat) $("spellbookButton").click();
      break;
    case "c":
      if (!event.repeat) $("craftButton").click();
      break;
    case "escape":
      if (!event.repeat) closePanels();
      break;
  }

  if (code === "Space" && !event.repeat) attack();
  updateKeyboardStick();
});
...
window.addEventListener("keyup", event => {
  ...
  switch (key) {
    case "w":
    case "arrowup":
      keyboardMovement.up = false;
      break;
    case "s":
    case "arrowdown":
      keyboardMovement.down = false;
      break;
    case "a":
    case "arrowleft":
...
const updatePlayerBeforeKeyboard = updatePlayer;
updatePlayer = function(dt) {
  updateKeyboardStick();
  updatePlayerBeforeKeyboard(dt);
};
