...
const SAVE_KEY = "proyecto_ultra_v4_5_3";
const LEGACY_SAVE_KEYS=["proyecto_ultra_v4_5_2","proyecto_ultra_v4_5_1","proyecto_ultra_v4_5","proyecto_ultra_v4_4","proyecto_ultra_v4_3","proyecto_ultra_v4_2","proyecto_ultra_v4_1","proyecto_ultra_v4"];
...
/* =====================================================================
   V4.5 — ESTABILIZACIÓN Y BALANCE
   ===================================================================== */

// ---------- Estado persistente ----------
player.attributeStates=player.attributeStates||{strength:"up",dexterity:"up",intelligence:"up"};
player.equippedTechniques=player.equippedTechniques||{60:null,80:null};
player.mapView=player.mapView||{zoom:1,panX:0,panY:0};
const merchantStocks={};

// ---------- Oro con icono claramente dorado ----------
function goldMarkup(value){return `<i class="gold-icon"></i>${Math.floor(value).toLocaleString("es-CL")}`}

// ---------- Reparación: ocultar completamente cuando no corresponde ----------
const renderEquipmentDetailsV45base=renderEquipmentDetails;
renderEquipmentDetails=function(slot,it){
 renderEquipmentDetailsV45base(slot,it);
 const box=$("equipmentDetails");
 if(!it){
  box.innerHTML=`<div class="details-empty"><div><h3>${SLOT_LABELS?.[slot]||"Espacio vacío"}</h3><p>Selecciona un objeto compatible de la mochila.</p></div></div>`;
  return
 }
 if(isDurable(it)&&it.durability<it.maxDurability){
  const actions=box.querySelector(".actions");
  if(actions&&!box.querySelector("[data-repair-v45]")){
   const b=document.createElement("button");b.dataset.repairV45="1";b.textContent=`Reparar (${repairCost(it)} oro)`;b.onclick=()=>repairItem(it);actions.prepend(b)
  }
 }else{
  box.querySelectorAll("[data-repair-v45],button").forEach(b=>{if(/^Reparar/.test(b.textContent))b.remove()})
 }
};
const renderInventoryDetailsV45base=renderInventoryDetails;
renderInventoryDetails=function(it,i){
 renderInventoryDetailsV45base(it,i);
 const box=$("inventoryDetails");
 box.querySelectorAll("button").forEach(b=>{if(/^Reparar/.test(b.textContent)&&(!isDurable(it)||it.durability>=it.maxDurability))b.remove()});
 if(isDurable(it)&&it.durability<it.maxDurability&&!box.querySelector("[data-repair-v45]")){
  const actions=box.querySelector(".actions");
  if(actions){const b=document.createElement("button");b.dataset.repairV45="1";b.textContent=`Reparar (${repairCost(it)} oro)`;b.onclick=()=>repairItem(it);actions.prepend(b)}
 }
};

// ---------- Multitáctil: cada control preserva el joystick ----------
function safeActionButton(id,handler){
 const el=$(id);if(!el)return;
 el.onclick=null;
 const run=e=>{e.preventDefault();e.stopPropagation();handler()};
 el.addEventListener("pointerdown",run,{passive:false})
}
function bindIndependentActions(){
 safeActionButton("battleModeButton",toggleBattleMode);
 safeActionButton("interactButton",interact);
 safeActionButton("potionButton",potion);
 safeActionButton("magicButton",magic);
 safeActionButton("healButton",heal);
 document.querySelectorAll("[data-spell-slot]").forEach((b,i)=>{
  b.onclick=null;b.addEventListener("pointerdown",e=>{e.preventDefault();e.stopPropagation();castSpell(player.equippedSpells[i])},{passive:false})
 })
}

// ---------- Barra de seis magias siempre visible ----------
function forceQuickbarVisible(){
 const q=$("spellQuickbar");if(q){q.style.display="grid";q.classList.remove("hidden")}
}
const toggleBattleModeV45base=toggleBattleMode;
toggleBattleMode=function(){
 toggleBattleModeV45base();
 forceQuickbarVisible();
 if(!battleMode){
  targetEnemy=null;player.charging=false;player.chargeTime=0;
  $("attackButton")?.classList.remove("charging");
  $("targetStatus").textContent="Sin objetivo"
 }
};

// ---------- Proyectiles: target real prevalece siempre ----------
function hasValidSpellTarget(){return !!(targetEnemy&&targetEnemy.alive)}
function spellNeedsTarget(s){return !!s?.offensive}
...
function launchHomingSpellV45(spell,target,damage){
 if(!target?.alive)return false;
 projectiles.push({
  kind:"homingSpellV45",x:player.x,y:player.y,z:20,targetId:target.uid||target.spawnId||null,
  target,spell,damage,speed:Math.max(10,spell.projectileSpeed||11),life:5,
  color:spell.icon==="🧊"?"#aee8ff":spell.name.includes("Rayo")||spell.icon==="⚡"?"#d8f4ff":"#ff8a45",owner:"player"
 });
 return true
}
const castSpellV45base=castSpell;
castSpell=function(id){
 if(!id)return;
 const s=spellDefs[id];if(!s)return;
 const selected=hasValidSpellTarget()?targetEnemy:null;
 if(selected&&spellNeedsTarget(s)){
  if(!hasLineOfSight(player,selected))return toast("No tienes línea de visión.");
  const reason=canCastSpell(s,id);if(reason)return toast(reason);
  const fatigue=fatigueLevel(id),cost=Math.ceil(s.cost*(1+fatigue*BALANCE.fatigueManaStep));
  player.mana-=cost;player.spellGlobalCd=BALANCE.spellGlobalCooldown;
  player.spellCooldowns[id]=cooldownAfterReduction(s.cooldown||.35);
  ...
  applyFatigue(id);decayOtherFatigue(id);player.castAnim=1;
  const power=(1-fatigue*BALANCE.fatiguePowerStep)*(1+buffValue("magicDamage")/100);
  const damage=Math.round(((s.damage||0)+skills.magery.value*.22+player.attributes.intelligence*.09)*power);
  if(id==="chainLightning"){
   const chain=[selected,...enemies.filter(e=>e.alive&&e!==selected&&dist(e,selected)<5).slice(0,3)];
   chain.forEach((e,i)=>setTimeout(()=>resolveSpellHit(e,s,Math.round(damage*(1-i*.16))),i*110));
   AudioSystem.lightning();renderQuickbar();return
  }
  if(["curse"].includes(id)){
   launchHomingSpellV45(s,selected,0);AudioSystem.tone(280,.18,"sine",.07,-80,"magic");renderQuickbar();return
  }
  launchHomingSpellV45(s,selected,damage);
  if(id==="iceberg")AudioSystem.ice();else if(["spark","lightning"].includes(id))AudioSystem.lightning();else AudioSystem.fire();
  gain(s.skill,.55,1);renderQuickbar();return
 }
 return castSpellV45base(id)
};
const updateEffectsV45base=updateEffects;
updateEffects=function(dt){
 // Resolve new homing projectiles first and remove them from the generic pass.
 for(const p of projectiles){
  if(p.kind!=="homingSpellV45")continue;
  const t=p.target;
  if(!t?.alive){p.life=0;continue}
  const dx=t.x-p.x,dy=t.y-p.y,d=Math.hypot(dx,dy)||1,step=p.speed*dt;
  const nx=p.x+dx/d*step,ny=p.y+dy/d*step;
  if(blocked(nx,ny)){p.life=0;burst(p.x,p.y,p.color,7);continue}
  p.x=nx;p.y=ny;p.z=19+Math.sin(performance.now()/85)*3;p.life-=dt;
  if(d<.5){
   resolveSpellHit(t,p.spell,p.damage);
   p.life=0;AudioSystem.impact()
  }
 }
 const modern=projectiles.filter(p=>p.kind==="homingSpellV45");
 projectiles.splice(0,projectiles.length,...projectiles.filter(p=>p.kind!=="homingSpellV45"));
 updateEffectsV45base(dt);
 const remainingModern=modern.filter(p=>p.life>0);
 projectiles.push(...remainingModern)
};

// ---------- Precisión y daño: espada fiable, arco dependiente de distancia/destreza ----------
function rangedAccuracy(e,st){
 const d=dist(player,e),skill=skills.archery.value,dex=player.attributes.dexterity;
 const moving=player.state==="walk"?14:0;
 const targetMove=e.state==="chase"||e.state==="wander"?6:0;
 const distancePenalty=Math.max(0,d-4)*3.1;
 return clamp(35+skill*.72+dex*.23+(st.accuracy||0)-moving-targetMove-distancePenalty,22,91)
}
function meleeAccuracy(skill,e,st){
 const base={swords:82,fencing:86,maces:72,axes:68,spears:78,unarmed:77}[skill]||76;
 return clamp(base+skills[skill].value*.17+player.attributes.dexterity*.08+(st.accuracy||0)-e.level*1.4,48,97)
}
const executeAttackV45base=executeAttack;
executeAttack=function(charged=false,manual=false){
 if(!battleMode||panelOpen()||player.attackCd>0)return false;
 const e=validTarget();if(!e)return manual&&toast("Selecciona un objetivo.");
 const skill=currentCombatSkill(),weapon=currentWeapon(),st=equipStats();
 if(!weaponReady())return toast("Tu arma necesita reparación.");
 const range=(st.range||1.75)+(charged?.25:0),d=dist(player,e);
 if(d>range)return manual&&toast("El objetivo está fuera de alcance.");
 const staminaCost=(st.staminaCost||8)*(charged?2.05:1);
 if(!spendStamina(staminaCost))return false;
 player.aimX=e.x-player.x;player.aimY=e.y-player.y;player.dirX=player.aimX;player.dirY=player.aimY;
 ...
 if(skill==="archery"){
  const ix=ammoIndex();if(ix<0){player.stamina+=staminaCost;return toast("No tienes flechas.")}
  removeAmount(ix,1);
  const hitChance=rangedAccuracy(e,st),roll=Math.random()*100;
  const damage=Math.round((rand(st.damageMin,st.damageMax)*.72+skills.archery.value*.10+player.attributes.dexterity*.035)*(charged?1.45:1));
  const l=Math.hypot(player.aimX,player.aimY)||1;
  projectiles.push({kind:"arrowV45",x:player.x,y:player.y,z:20,vx:player.aimX/l,vy:player.aimY/l,speed:12,range:st.range||16,traveled:0,color:"#d6c49a",life:3,
   damage,poise:charged?15:4,owner:"player",intended:e,willHit:roll<hitChance});
 }else{
  const hitChance=meleeAccuracy(skill,e,st);
  const critChance=clamp(4+(st.critical||0)+skills[skill].value*.035,4,30);
  const base=rand(st.damageMin,st.damageMax)+skills[skill].value*.16+player.attributes.strength*.055;
  const crit=Math.random()*100<critChance;
  let dmg=Math.round(base*(charged?1.72:1)*(crit?1.55:1));
  setTimeout(()=>{
 ...
 if(weapon)wearItem(weapon,charged?1.25:.35);
 gain(skill,charged?.7:.35,e.level);gainAttribute(skill==="fencing"||skill==="archery"?"dexterity":"strength",charged?.12:.05);
 AudioSystem.sword();return true
};

// Arrow miss resolves near target instead of guaranteed hit.
const updateEffectsArrowBase=updateEffects;
updateEffects=function(dt){
 for(const p of projectiles){
  if(p.kind!=="arrowV45")continue;
  const t=p.intended;
  if(!t?.alive){p.life=0;continue}
  const tx=t.x+(p.willHit?0:.9),ty=t.y+(p.willHit?0:.55);
  const dx=tx-p.x,dy=ty-p.y,d=Math.hypot(dx,dy)||1,step=p.speed*dt;
  p.x+=dx/d*step;p.y+=dy/d*step;p.traveled+=step;p.life-=dt;p.z=18;
  if(blocked(p.x,p.y)||p.traveled>=p.range){p.life=0;continue}
  if(d<.42){
   if(p.willHit)damageEnemy(t,p.damage,p.poise||4);else floatText(t.x,t.y,"FALLO","#d7d7d7");
   p.life=0
  }
 }
 const arrows=projectiles.filter(p=>p.kind==="arrowV45");
 projectiles.splice(0,projectiles.length,...projectiles.filter(p=>p.kind!=="arrowV45"));
 updateEffectsArrowBase(dt);
 projectiles.push(...arrows.filter(p=>p.life>0))
};

// ---------- Cooldown visual robusto con conic-gradient ----------
function applyCooldownVisual(button,remaining,total){
 if(!button)return;
 const pct=total>0?clamp(remaining/total,0,1):0,angle=Math.round(pct*360);
 button.style.setProperty("--cooldown-angle",`${angle}deg`);
 button.classList.toggle("cooldown-active",remaining>0);
 const mask=button.querySelector(".cooldown-mask,.ability-cooldown");
 if(mask)mask.style.setProperty("--cooldown-angle",`${angle}deg`);
 const txt=button.querySelector(".cooldown-time,.ability-time");
 if(txt)txt.textContent=remaining>0?(remaining<1?remaining.toFixed(1):Math.ceil(remaining)):"";
}
const renderQuickbarV45base=renderQuickbar;
renderQuickbar=function(){
 renderQuickbarV45base();
 forceQuickbarVisible();
 document.querySelectorAll("#spellQuickbar button").forEach((b,i)=>{
  const id=player.equippedSpells[i],s=id?spellDefs[id]:null,rem=id?spellCooldownRemaining(id):0;
  applyCooldownVisual(b,rem,s?(s.cooldown||.35):1)
 })
};
const updateHudV45base=updateHud;
updateHud=function(){
 updateHudV45base();
 $("hudGold").innerHTML=goldMarkup(player.gold);
 applyCooldownVisual($("potionButton"),player.potionCd,BALANCE.potionCooldown);
 applyCooldownVisual($("special60"),player.specialCooldowns[60]||0,BALANCE.special60.cooldown);
 applyCooldownVisual($("special80"),player.specialCooldowns[80]||0,BALANCE.special80.cooldown);
};

// ---------- Atributos: subir, mantener o bajar ----------
function attributeTotal(){return player.attributes.strength+player.attributes.dexterity+player.attributes.intelligence}
function attributeLabel(id){return({strength:"Fuerza",dexterity:"Destreza",intelligence:"Inteligencia"})[id]}
const gainAttributeV45base=gainAttribute;
gainAttribute=function(id,amount){
 if(player.attributeStates[id]!=="up")return;
 if(player.attributes[id]>=150)return;
 const total=attributeTotal();
 if(total>=300){
  const donor=Object.keys(player.attributes).find(k=>player.attributeStates[k]==="down"&&player.attributes[k]>10);
  if(!donor)return;
  player.attributes[donor]=Math.max(10,player.attributes[donor]-.1)
 }
 return gainAttributeV45base(id,amount)
};
function renderAttributes(){
 derived();const a=player.attributes,total=attributeTotal(),st=equipStats();
 $("attributesContent").innerHTML=`
  <div class="attribute-summary">
   <article><small>Total</small><strong>${total.toFixed(1)}</strong><span>/ 300</span></article>
   <article><small>Vida máxima</small><strong>${Math.round(player.maxHealth)}</strong></article>
   <article><small>Maná máximo</small><strong>${Math.round(player.maxMana)}</strong></article>
  </div>
  ${Object.keys(a).map(id=>`<div class="attribute-row">
   <strong>${attributeLabel(id)}</strong><b>${a[id].toFixed(1)}</b>
   <div class="attribute-state">
    <button data-attr="${id}" data-attr-state="up" class="${player.attributeStates[id]==="up"?"active":""}">↑</button>
    <button data-attr="${id}" data-attr-state="lock" class="${player.attributeStates[id]==="lock"?"active":""}">—</button>
    <button data-attr="${id}" data-attr-state="down" class="${player.attributeStates[id]==="down"?"active":""}">↓</button>
   </div>
  </div>`).join("")}
  <div class="derived-grid">
   <div class="stat"><span>Precisión espada</span><strong>${Math.round(meleeAccuracy("swords",{level:1},st))}%</strong></div>
   <div class="stat"><span>Precisión arco a 6 casillas</span><strong>${Math.round(clamp(35+skills.archery.value*.72+a.dexterity*.23-6.2,22,91))}%</strong></div>
   <div class="stat"><span>Equilibrio</span><strong>${Math.round(player.poise)} / ${player.maxPoise}</strong></div>
   <div class="stat"><span>Resistencia mágica</span><strong>${Math.round((st.magicResistance||0)+skills.magicResistance.value*.2)}</strong></div>
  </div>`;
 $("attributesContent").querySelectorAll("[data-attr-state]").forEach(b=>b.onclick=()=>{player.attributeStates[b.dataset.attr]=b.dataset.attrState;renderAttributes()})
}

// ---------- Técnicas 60/80 equipables ----------
const TECHNIQUES={
 swords:{60:"Golpe poderoso",80:"Ignorar armadura"},fencing:{60:"Estocada precisa",80:"Danza de cuchillas"},
 maces:{60:"Golpe quebrador",80:"Impacto sísmico"},axes:{60:"Hendidura",80:"Torbellino"},
 spears:{60:"Embestida",80:"Empalamiento"},archery:{60:"Disparo perforante",80:"Lluvia de flechas"},
 unarmed:{60:"Golpe aturdidor",80:"Llave de combate"},shieldMastery:{60:"Guardia firme",80:"Contraataque"},
 endurance:{60:"Segundo aliento",80:"Inquebrantable"},magery:{60:"Nova de hielo",80:"Cadena de relámpagos"},
 healing:{60:"Regeneración",80:"Escudo vital"},arcaneKnowledge:{60:"Lectura de debilidad",80:"Sobrecarga arcana"},
 magicResistance:{60:"Velo arcano",80:"Anulación"}
};
function unlockedTechniques(level){
 return Object.entries(TECHNIQUES).filter(([skill,n])=>skills[skill]?.value>=level).map(([skill,n])=>({skill,name:n[level],level}))
}
function renderTechniques(){
 [60,80].forEach(level=>{
  const box=$(level===60?"techniques60":"techniques80");box.innerHTML="";
  const available=unlockedTechniques(level);
  if(!available.length){box.innerHTML=`<p>Aún no has alcanzado ${level} en una habilidad compatible.</p>`;return}
  available.forEach(t=>{
   const b=document.createElement("button");b.className="list-row"+(player.equippedTechniques[level]?.skill===t.skill?" technique-equipped":"");
   b.innerHTML=`<span>${level}</span><div><strong>${t.name}</strong><small>${skillInfo[t.skill].name} ${skills[t.skill].value.toFixed(1)}</small></div><b>Equipar</b>`;
   b.onclick=()=>{player.equippedTechniques[level]=t;renderTechniques();updateSpecialButtons()};box.appendChild(b)
  })
 })
}
updateSpecialButtons=function(){
 [60,80].forEach(level=>{
  const b=$(level===60?"special60":"special80"),t=player.equippedTechniques[level],available=unlockedTechniques(level);
  if(t&&skills[t.skill]?.value<level)player.equippedTechniques[level]=null;
  const current=player.equippedTechniques[level];
  b.classList.toggle("locked",!current);
  b.querySelector("span").textContent=level;
  b.querySelector("small").textContent=current?current.name:(available.length?"Equipar":"Bloqueado")
 })
};
useSpecial=function(level){
 const selected=player.equippedTechniques[level];
 if(!selected){openPanel("techniquesPanel");renderTechniques();return}
 const cfg=level===60?BALANCE.special60:BALANCE.special80;
 if(player.specialCooldowns[level]>0)return toast("La técnica aún se está recuperando.");
 if(player.mana<cfg.mana)return toast("No tienes suficiente maná.");
 if(player.stamina<cfg.stamina)return toast("No tienes suficiente aguante.");
 player.mana-=cfg.mana;player.stamina-=cfg.stamina;player.specialCooldowns[level]=cfg.cooldown;
 const saved=highestCombatSkill;
 highestCombatSkill=()=>selected.skill;
 try{const original=player.specialCooldowns[level];player.specialCooldowns[level]=0;useSpecialV44base(level);player.specialCooldowns[level]=original}catch(e){console.warn(e)}
 highestCombatSkill=saved;AudioSystem.impact()
};

// ---------- Mapa: zoom, paneo y nombres de regiones descubiertas ----------
let mapDrag=null;
function knownRegion(r){return discovered.regions.has(r.name)}
function drawMap(){
 const c=mapCanvas,w=c.width,h=c.height,view=player.mapView,scale=Math.min(w,h)/WORLD*view.zoom;
 mapCtx.clearRect(0,0,w,h);mapCtx.save();
 mapCtx.translate(w/2+view.panX,h/2+view.panY);
 mapCtx.scale(view.zoom,view.zoom);
 mapCtx.translate(-WORLD/2,-WORLD/2);
 const sx=w/WORLD,sy=h/WORLD;
 for(let y=0;y<WORLD;y+=3)for(let x=0;x<WORLD;x+=3){
  const known=explored.has(`${Math.floor(x/3)},${Math.floor(y/3)}`);
  mapCtx.fillStyle=known?tileColor(tileAt(x,y)):"#070a0b";
  mapCtx.fillRect(x*sx/view.zoom,y*sy/view.zoom,sx*3/view.zoom+1,sy*3/view.zoom+1)
 }
 mapCtx.restore();
 // Region labels in screen coordinates.
 regions.forEach(r=>{
  if(!knownRegion(r))return;
  const rx=(r.x+r.w/2-WORLD/2)*scale+w/2+view.panX;
  const ry=(r.y+r.h/2-WORLD/2)*scale+h/2+view.panY;
  mapCtx.font="bold 12px Georgia";mapCtx.textAlign="center";mapCtx.lineWidth=3;mapCtx.strokeStyle="#071016";
  mapCtx.strokeText(r.name,rx,ry);mapCtx.fillStyle="#f0d889";mapCtx.fillText(r.name,rx,ry)
 });
 const px=(player.x-WORLD/2)*scale+w/2+view.panX,py=(player.y-WORLD/2)*scale+h/2+view.panY;
 mapCtx.fillStyle="#fff";mapCtx.beginPath();mapCtx.arc(px,py,5,0,Math.PI*2);mapCtx.fill();
 $("mapZoomLabel").textContent=`${Math.round(view.zoom*100)}%`
}
function setMapZoom(delta){
 player.mapView.zoom=clamp(player.mapView.zoom+delta,.75,3);drawMap()
}
function centerMap(){
 const scale=Math.min(mapCanvas.width,mapCanvas.height)/WORLD*player.mapView.zoom;
 player.mapView.panX=-(player.x-WORLD/2)*scale;player.mapView.panY=-(player.y-WORLD/2)*scale;drawMap()
}

// ---------- Tiendas: stock persistente y anillos exactamente con 1 propiedad ----------
function makeMerchantRing(){
 const it=makeItem("ring");
 it.rarity="uncommon";
 it.buffs=randomBuffs(1,"rare").slice(0,1);
 if(!it.buffs.length)it.buffs=[{id:"manaRegen",name:"Regeneración de maná",value:Math.round(rand(.5,4)*10)/10}];
 it.value=Math.round(180+it.buffs[0].value*85);
 return it
}
function stockFor(role){
 if(merchantStocks[role])return merchantStocks[role];
 merchantStocks[role]=(shops[role]?.items||[]).map(id=>{
  if(id==="ring")return makeMerchantRing();
  const it=makeItem(id);
  if(it.slot&&id!=="ring")return enhanceItem(it,.35);
  return it
 });
 return merchantStocks[role]
}
renderShop=function(){
 const list=$("shopList");list.innerHTML="";$("shopBuyTab").classList.toggle("active",currentShopMode==="buy");$("shopSellTab").classList.toggle("active",currentShopMode==="sell");
 const arr=currentShopMode==="buy"?stockFor(currentShop):player.inventory;
 arr.forEach((it,i)=>{
  const r=document.createElement("button");r.className="list-row"+(selectedShop===i?" selected":"");
  const price=currentShopMode==="buy"?it.value:Math.max(1,Math.floor(it.value*.45)),owned=countItem(it.id),amount=it.stack?it.count:1;
  r.innerHTML=`<span>${it.icon}</span><div><strong class="rarity-${it.rarity}">${it.name}${amount>1?` ×${amount}`:""}</strong><small>${it.type}${currentShopMode==="buy"?` · <span class="shop-owned">Tienes: ${owned}</span>`:""}</small></div><b>${price} oro</b>`;
  r.onclick=()=>{selectedShop=i;shopQuantity=1;renderShop();renderShopDetails(i,it,price)};list.appendChild(r)
 })
};
const shopActionV45base=shopAction;
shopAction=function(i,it,price,qty=1){
 if(currentShopMode==="buy"){
  const total=price*qty;if(player.gold<total)return toast("No tienes suficiente oro.");
  player.gold-=total;
  if(it.stack)addItem(makeItem(it.id,{count:qty}));
  else{
   const exact=JSON.parse(JSON.stringify(it));exact.uid=uid();normalizeItem(exact);addItem(exact);
   merchantStocks[currentShop].splice(i,1)
  }
  toast(`Compraste ${qty} ${it.name}.`);selectedShop=-1;renderShop();AudioSystem.uiClick();return
 }
 return shopActionV45base(i,it,price,qty)
};

// ---------- Equipo móvil: estado vacío pequeño y mejor posición ----------
const renderEquipmentV45base=renderEquipment;
renderEquipment=function(){
 renderEquipmentV45base();
 if(selectedEquipment==null&&selectedEquipInventory<0)$("equipmentDetails").innerHTML='<div class="details-empty"><div><h3>Selecciona un objeto</h3><p>Verás aquí sus estadísticas y acciones.</p></div></div>';
 $("equipmentGold").innerHTML=goldMarkup(player.gold);
};

// ---------- Guardado de nuevos sistemas ----------
const saveV45base=save;
save=function(){
 saveV45base();
 try{
  const d=JSON.parse(localStorage.getItem(SAVE_KEY)||"{}");
  d.version=4.5;d.player.attributeStates=player.attributeStates;d.player.equippedTechniques=player.equippedTechniques;d.player.mapView=player.mapView;
  d.merchantStocks=merchantStocks;
  localStorage.setItem(SAVE_KEY,JSON.stringify(d))
 }catch(e){console.warn(e)}
};
const loadV45base=load;
load=function(){
 loadV45base();
 player.attributeStates=player.attributeStates||{strength:"up",dexterity:"up",intelligence:"up"};
 player.equippedTechniques=player.equippedTechniques||{60:null,80:null};
 player.mapView=player.mapView||{zoom:1,panX:0,panY:0};
 try{
  const raw=localStorage.getItem(SAVE_KEY);if(raw){const d=JSON.parse(raw);Object.assign(merchantStocks,d.merchantStocks||{})}
 }catch(e){}
};

// ---------- Bind final ----------
const bindV45base=bind;
bind=function(){
 bindV45base();
 bindIndependentActions();forceQuickbarVisible();
 $("playerHud").onclick=()=>{openPanel("attributesPanel");renderAttributes()};
 $("special60").addEventListener("contextmenu",e=>{e.preventDefault();openPanel("techniquesPanel");renderTechniques()});
 $("special80").addEventListener("contextmenu",e=>{e.preventDefault();openPanel("techniquesPanel");renderTechniques()});
 // Tocar una técnica bloqueada/ausente abre el selector.
 ["special60","special80"].forEach((id,idx)=>$(id).addEventListener("dblclick",()=>{openPanel("techniquesPanel");renderTechniques()}));
 $("mapZoomIn").onclick=()=>setMapZoom(.25);$("mapZoomOut").onclick=()=>setMapZoom(-.25);$("mapCenter").onclick=centerMap;
 $("mapCanvas").addEventListener("wheel",e=>{e.preventDefault();setMapZoom(e.deltaY<0?.15:-.15)},{passive:false});
 $("mapCanvas").addEventListener("pointerdown",e=>{mapDrag={id:e.pointerId,x:e.clientX,y:e.clientY,px:player.mapView.panX,py:player.mapView.panY};$("mapCanvas").setPointerCapture?.(e.pointerId)});
 $("mapCanvas").addEventListener("pointermove",e=>{if(!mapDrag||mapDrag.id!==e.pointerId)return;player.mapView.panX=mapDrag.px+e.clientX-mapDrag.x;player.mapView.panY=mapDrag.py+e.clientY-mapDrag.y;drawMap()});
 $("mapCanvas").addEventListener("pointerup",()=>mapDrag=null);$("mapCanvas").addEventListener("pointercancel",()=>mapDrag=null);
};


/* =====================================================================
   V4.5.1 — ANILLOS, MAGIA DIRIGIDA Y COMPRAS SEGURAS
   ===================================================================== */

// ---------- Selector de mano para anillos ----------
let pendingRingEquipIndex=-1;

function ringSlotLabel(slot){
 return slot==="ring1"?"Anillo izquierdo":"Anillo derecho"
}
function ringStatus(slot){
 const it=player.equipment[slot];
 return it?`${it.name}${it.buffs?.length?` · ${it.buffs[0].name} ${it.buffs[0].value}%`:""}`:"Vacío"
}
function openRingHandSelector(inventoryIndex){
 const it=player.inventory[inventoryIndex];
 if(!it||it.type!=="jewelry"||it.slot!=="ring")return false;
 pendingRingEquipIndex=inventoryIndex;
 $("ringHandDescription").textContent=`¿Dónde quieres equipar ${it.name}?`;
 $("ringLeftStatus").textContent=ringStatus("ring1");
 $("ringRightStatus").textContent=ringStatus("ring2");
 $("equipRingLeft").classList.toggle("occupied",!!player.equipment.ring1);
 $("equipRingRight").classList.toggle("occupied",!!player.equipment.ring2);
 openPanel("ringHandPanel");
 return true
}
function equipRingToSlot(slot){
 const i=pendingRingEquipIndex,it=player.inventory[i];
 if(i<0||!it||it.type!=="jewelry"||it.slot!=="ring"){
  pendingRingEquipIndex=-1;closePanels();return toast("El anillo ya no está disponible.")
 }
 const old=player.equipment[slot];
 const equipped=player.inventory.splice(i,1)[0];
 equipped.slot=slot;
 ...
 pendingRingEquipIndex=-1;
 closePanels();derived();renderInventory();renderEquipment();
 if(typeof captureLoadout==="function")captureLoadout(player.activeLoadout);
 toast(`${equipped.name} equipado en ${ringSlotLabel(slot).toLowerCase()}.`)
}
const equipFromInventoryV451Base=equipFromInventory;
equipFromInventory=function(i){
 const it=player.inventory[i];
 if(it?.type==="jewelry"&&it.slot==="ring")return openRingHandSelector(i);
 return equipFromInventoryV451Base(i)
};
if(typeof SLOT_LABELS!=="undefined"){
 SLOT_LABELS.ring1="Anillo izquierdo";
 SLOT_LABELS.ring2="Anillo derecho";
}

// ---------- Clasificación explícita de hechizos ----------
function normalizeSpellTargeting(){
 Object.entries(spellDefs).forEach(([id,s])=>{
  if(["heal","regeneration","vitalShield","arcaneBarrier","physicalProtection"].includes(id))s.targetMode="self";
  else if(id==="iceNova")s.targetMode="areaSelf";
  else if(id==="chainLightning")s.targetMode="targetedChain";
  else if(s.offensive)s.targetMode="targeted";
  else s.targetMode=s.targetMode||"self";
 });
}
normalizeSpellTargeting();

function selectedLivingTarget(){
 return targetEnemy&&targetEnemy.alive?targetEnemy:null
}
function prepareSpellCost(id,s){
 const reason=canCastSpell(s,id);
 if(reason)return {ok:false,reason};
 const fatigue=fatigueLevel(id);
 return {
  ok:true,
  fatigue,
  cost:Math.ceil(s.cost*(1+fatigue*BALANCE.fatigueManaStep)),
  power:(1-fatigue*BALANCE.fatiguePowerStep)*(1+buffValue("magicDamage")/100)
 }
}
function spendSpellResources(id,s,prepared){
 player.mana-=prepared.cost;
 player.spellGlobalCd=BALANCE.spellGlobalCooldown;
 player.spellCooldowns[id]=cooldownAfterReduction(s.cooldown||.35);
 if(s.advanced)consume("arcaneReagent",1);
 applyFatigue(id);decayOtherFatigue(id);player.castAnim=1;
}
function castDirectionalProjectile(s,damage){
 const l=Math.hypot(player.dirX,player.dirY)||1;
 projectiles.push({
  kind:"freeSpell",x:player.x,y:player.y,z:20,
  vx:player.dirX/l,vy:player.dirY/l,speed:s.projectileSpeed||11,
  range:s.range||18,traveled:0,life:3,color:"#ff874a",
  owner:"player",damage,poise:s.poise||5
 })
}
function castTargetedSpell(id,s,target,damage){
 if(id==="chainLightning"){
  const chain=[target,...enemies.filter(e=>e.alive&&e!==target&&dist(e,target)<5).slice(0,3)];
  chain.forEach((e,i)=>setTimeout(()=>resolveSpellHit(e,s,Math.round(damage*(1-i*.16))),i*110));
  AudioSystem.lightning();return
 }
 launchHomingSpellV45(s,target,damage);
 if(id==="iceberg")AudioSystem.ice();
 else if(["spark","lightning"].includes(id))AudioSystem.lightning();
 else if(id==="curse")AudioSystem.tone(280,.18,"sine",.07,-80,"magic");
 else AudioSystem.fire()
}

castSpell=function(id){
 if(!id)return;
 const s=spellDefs[id];if(!s)return;
 const prep=prepareSpellCost(id,s);
 if(!prep.ok)return toast(prep.reason);
 const target=selectedLivingTarget();
 const damage=Math.round(((s.damage||0)+skills.magery.value*.22+player.attributes.intelligence*.09)*prep.power);

 if(s.targetMode==="targeted"||s.targetMode==="targetedChain"){
  if(target){
   if(!hasLineOfSight(player,target))return toast("No tienes línea de visión.");
   spendSpellResources(id,s,prep);
   castTargetedSpell(id,s,target,damage);
   gain(s.skill,.55,1);renderQuickbar();return
  }
  spendSpellResources(id,s,prep);
  castDirectionalProjectile(s,damage);
  if(id==="iceberg")AudioSystem.ice();
  else if(["spark","lightning"].includes(id))AudioSystem.lightning();
  else AudioSystem.fire();
  gain(s.skill,.55,1);renderQuickbar();return
 ...
 spendSpellResources(id,s,prep);
 if(id==="heal"){
  player.health=Math.min(player.maxHealth,player.health+Math.round((42+skills.healing.value*.35)*prep.power));
  floatText(player.x,player.y,"CURACIÓN","#76e89b");AudioSystem.heal()
 }else if(id==="regeneration"){player.regen=8;AudioSystem.heal()}
 else if(id==="vitalShield"){player.vitalShield=100;AudioSystem.heal()}
 else if(id==="arcaneBarrier"){player.magicWard=8;AudioSystem.tone(520,.14,"sine",.06,160,"magic")}
 else if(id==="physicalProtection"){player.guardBuff=8;AudioSystem.uiClick()}
 else if(id==="iceNova"){
  enemies.filter(e=>e.alive&&dist(e,player)<3).forEach(e=>resolveSpellHit(e,s,Math.round((30+skills.magery.value*.3)*prep.power)));
  AudioSystem.ice()
 }
 gain(s.skill,.55,1);renderQuickbar()
};
...
function inventoryHasSpaceFor(it){
 if(it.stack&&player.inventory.some(x=>x.id===it.id&&x.stack))return true;
 return player.inventory.length<36
}
function deepClonePurchasedItem(it){
 const clone=JSON.parse(JSON.stringify(it));
 clone.uid=uid();
 normalizeItem(clone);
 if(clone.type==="jewelry"&&["ring","ring1","ring2"].includes(clone.slot))clone.slot="ring";
 return clone
}
function inventorySignature(){
 return JSON.stringify(player.inventory.map(it=>({
  uid:it.uid,id:it.id,count:it.count,rarity:it.rarity,buffs:it.buffs,durability:it.durability
 })))
}
function deliverPurchasedItem(it,qty){
 if(!inventoryHasSpaceFor(it))return {ok:false,error:"No tienes espacio en el inventario."};
 const before=inventorySignature();
 if(it.stack)addItem(makeItem(it.id,{count:qty}));
 else addItem(deepClonePurchasedItem(it));
 const after=inventorySignature();
 if(before===after)return {ok:false,error:"No se pudo añadir el objeto al inventario."};
 return {ok:true}
}
function transactionalPurchase(stockIndex,it,price,qty){
 qty=it.stack?Math.max(1,Math.floor(qty)):1;
 const total=price*qty;
 if(player.gold<total)return {ok:false,error:"No tienes suficiente oro."};
 const goldBefore=player.gold;
 const inventoryBefore=JSON.parse(JSON.stringify(player.inventory));
 const stockBefore=(merchantStocks[currentShop]||[]).slice();
 try{
  const delivery=deliverPurchasedItem(it,qty);
  if(!delivery.ok)return delivery;
  player.gold-=total;
  if(!it.stack)merchantStocks[currentShop].splice(stockIndex,1);
  return {ok:true,total,qty}
 }catch(error){
  player.gold=goldBefore;
  player.inventory=inventoryBefore;
  merchantStocks[currentShop]=stockBefore;
  return {ok:false,error:"La compra no pudo completarse."}
 }
}
const shopActionV451SellBase=shopAction;
shopAction=function(i,it,price,qty=1){
 if(currentShopMode!=="buy")return shopActionV451SellBase(i,it,price,qty);
 const result=transactionalPurchase(i,it,price,qty);
 if(!result.ok)return toast(result.error);
 toast(`Compraste ${result.qty} ${it.name}.`);
 selectedShop=-1;renderShop();
 if(typeof renderInventory==="function")renderInventory();
 updateHud();AudioSystem.uiClick()
};

// ---------- Pruebas internas ----------
function selfTestV451(){
 normalizeSpellTargeting();
 return [
  ["fireball-targeted",spellDefs.fireball?.targetMode==="targeted"],
  ["explosion-targeted",spellDefs.explosion?.targetMode==="targeted"],
  ["iceberg-targeted",spellDefs.iceberg?.targetMode==="targeted"],
  ["curse-targeted",spellDefs.curse?.targetMode==="targeted"],
  ["poison-targeted",spellDefs.poison?.targetMode==="targeted"],
  ["ring-one-buff",makeMerchantRing().buffs?.length===1],
  ["ring-selector",!!$("ringHandPanel")&&!!$("equipRingLeft")&&!!$("equipRingRight")]
 ].map(([name,ok])=>({name,ok}))
}

const bindV451Base=bind;
bind=function(){
 bindV451Base();
 $("equipRingLeft").onclick=()=>equipRingToSlot("ring1");
 $("equipRingRight").onclick=()=>equipRingToSlot("ring2");
 $("cancelRingEquip").onclick=()=>{pendingRingEquipIndex=-1;closePanels()};
};


/* =====================================================================
   V4.5.2 — HUD DE EQUIPO ORDENADO
   ===================================================================== */

// Evitar información duplicada de velocidad dentro de detalles.
function stripDuplicateWeaponSpeed(container){
 if(!container)return;
 const profile=container.querySelector(".weapon-profile");
 if(!profile)return;
 const paragraphs=[...profile.querySelectorAll("p")];
 const speedRows=paragraphs.filter(p=>p.textContent.includes("Velocidad de ataque"));
 speedRows.slice(1).forEach(p=>p.remove())
}

// Garantizar que el avatar se dibuje cada vez que se abre o actualiza equipo.
function ensurePaperDollVisible(){
 const canvas=$("paperDollCanvas");
 if(!canvas)return;
 canvas.style.display="block";
 canvas.style.visibility="visible";
 canvas.style.opacity="1";
 if(typeof drawPaperDoll==="function")drawPaperDoll()
}

// Ajustar etiquetas largas para que no creen superposición.
function normalizeEquipmentSlotLabels(){
 const labels={
  head:"Cabeza",neck:"Cuello",chest:"Pecho",arms:"Brazos",gloves:"Guantes",
  pants:"Piernas",boots:"Botas",tunic:"Túnica",cape:"Capa",
  mainHand:"Arma",offHand:"Escudo / Grimorio",ring1:"Anillo I",ring2:"Anillo II"
 };
 document.querySelectorAll(".equipment-slot[data-slot]").forEach(slot=>{
  const small=slot.querySelector("small");
  if(small&&!slot.querySelector(".item-icon"))small.textContent=labels[slot.dataset.slot]||small.textContent
 })
}

// Reaplicar el layout tras renderizar el equipo.
const renderEquipmentV452Base=renderEquipment;
renderEquipment=function(){
 renderEquipmentV452Base();
 normalizeEquipmentSlotLabels();
 ensurePaperDollVisible();
 stripDuplicateWeaponSpeed($("equipmentDetails"));
...
// También limpiar duplicados al seleccionar objetos.
const renderEquipmentDetailsV452Base=renderEquipmentDetails;
renderEquipmentDetails=function(slot,it){
 renderEquipmentDetailsV452Base(slot,it);
 stripDuplicateWeaponSpeed($("equipmentDetails"))
};
const renderInventoryDetailsV452Base=renderInventoryDetails;
renderInventoryDetails=function(it,i){
 renderInventoryDetailsV452Base(it,i);
 stripDuplicateWeaponSpeed($("inventoryDetails"))
};

// Prueba interna de la disposición.
function selfTestEquipmentLayoutV452(){
 const slots=[...document.querySelectorAll(".equipment-slot[data-slot]")];
 const required=["head","neck","chest","arms","gloves","pants","boots","tunic","cape","mainHand","offHand","ring1","ring2"];
 return {
  avatar:!!$("paperDollCanvas"),
  slots:required.every(id=>slots.some(s=>s.dataset.slot===id)),
...
/* =====================================================================
   V4.5.3 — CORRECCIÓN DE CARGA
   ===================================================================== */

function refreshEquipmentVisualSafely(){
...
const bindV453Base=bind;
bind=function(){
 bindV453Base();
 const equipmentButton=$("equipmentButton")||$("inventoryButton");
 if(equipmentButton){
  equipmentButton.addEventListener("click",()=>{
   requestAnimationFrame(refreshEquipmentVisualSafely);
   setTimeout(refreshEquipmentVisualSafely,40)
  })
 }
 document.querySelectorAll('[data-tab="equipment"],[data-character-tab]').forEach(button=>{
  button.addEventListener("click",()=>requestAnimationFrame(refreshEquipmentVisualSafely))
 })
};

window.ultraDiagnostics=function(){
 return {
  canvas:!!$("gameCanvas"),
  player:typeof player!=="undefined",
  bind:typeof bind,
  loop:typeof loop,
  equipmentButton:!!($("equipmentButton")||$("inventoryButton")),
  paperDoll:!!$("paperDollCanvas"),
