(function(U){
  ...
  const layer=(actor,slot)=>{const item=actor.equipment?.[slot];return item&&U.itemDefs[item.id]?.visualLayer};
  const materialColors={Cuero:'#704a32',Cobre:'#8c5538',Hierro:'#8d98a1',Oro:'#d6ad3e',Mithril:'#76529b'};
  function equippedColor(actor,type,fallback){
    const items=Object.values(actor.equipment||{}).filter(Boolean);
    const found=items.map(it=>U.itemDefs[it.id]).find(d=>d&&d.type===type&&materialColors[d.material]);
    return found?materialColors[found.material]:fallback;
  }
  function weapon(actor){
    const item=actor.equipment?.weapon;
    const def=item&&U.itemDefs[item.id];
    if(!def)return equipped(actor,'shield')?'shieldOnly':'none';
  ...
  U.CharacterAdapter={
    getState(actor,options={}){
      ...
      if(beastPalettes[type]){
        return {beast:type==='boss'||type==='cryptboss'?'troll':type,palette:beastPalettes[type],time:actor.anim||0,dir:actor.facing||'right',action:(actor.attackAnim||0)>0?'attack':actor.moving?'walk':'idle',scale:actor.boss||actor.variant==='renowned'?1.22:1};
      }
      const player=actor===U.player;
      const source=player?palettes.player:(palettes[type]||palettes.vendor);
      const base={...source,armor:equippedColor(actor,'armor',source.armor),weapon:equippedColor(actor,'weapon',source.armor)};
      return {
        ...
        weapon:player?weapon(actor):'none',
        shield:player&&equipped(actor,'shield')&&!U.itemDefs[actor.equipment?.weapon?.id]?.twoHand&&!U.itemDefs[actor.equipment?.weapon?.id]?.ranged,
        hair:player?'ponytail':type==='guard'?'short':type==='healer'?'bun':'long',
        showClothes:!player||['head','neck','chest','arms','gloves','pants','boots'].some(slot=>layer(actor,slot)==='clothing'),
        clothes:player?{head:layer(actor,'head')==='clothing',neck:layer(actor,'neck')==='clothing',shirt:layer(actor,'chest')==='clothing',arms:layer(actor,'arms')==='clothing',gloves:layer(actor,'gloves')==='clothing',pants:layer(actor,'pants')==='clothing'}:{head:false,neck:false,shirt:true,arms:true,gloves:false,pants:true},
        equip:{helmet:player&&layer(actor,'head')==='armor',neck:player&&layer(actor,'neck')==='armor',torso:player&&layer(actor,'chest')==='armor',arms:player&&layer(actor,'arms')==='armor',gloves:player&&layer(actor,'gloves')==='armor',legs:player&&layer(actor,'pants')==='armor',boots:player&&layer(actor,'boots')==='armor'},
