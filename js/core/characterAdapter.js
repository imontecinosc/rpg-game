(function (U) {
  const palettes = {
    ...
    /* Mago humano: túnica arcana, distinta de todo lo demás para que se
       note a simple vista que no es un aldeano ni un bandido común. */
    mage: {
      gender: 'male',
      skin: '#a8826c',
      underwear: '#241f30',
  ...
  /* Misma prioridad que ORDEN_CUERPO (usada para elegir el SET de
     armadura): el pecho manda, y si no hay pecho se sigue buscando en el
     resto del cuerpo en este orden fijo. Antes se tomaba "la primera
     pieza que aparezca al iterar el objeto de equipo", un orden que
     cambia según qué se equipó/desequipó y en qué momento — por eso el
     color de TODA la armadura podía saltar al poner una pieza nueva que
     no tenía nada que ver. Con un orden fijo, solo cambiar el pecho (o
     la pieza más relevante puesta) cambia el color visible. */
  const ORDEN_COLOR_ARMADURA = ['chest', 'pants', 'head', 'arms', 'gloves', 'boots', 'neck', 'shield'];
  function equippedColor(actor, type, fallback) {
    if (type === 'armor') {
      for (const slot of ORDEN_COLOR_ARMADURA) {
        const item = actor.equipment?.[slot];
        const def = item && U.itemDefs[item.id];
        if (def && def.type === 'armor' && (def.visualColor || materialColors[def.material]))
          return def.visualColor || materialColors[def.material];
      }
      return fallback;
    }
  ...
  function weaponHands(actor) {
    ...
    let main = def.visualModel || 'none';
    if (def.visualModel) main = def.visualModel;
    else if (def.skill === 'Arco') main = 'bow';
  ...
  function weapon(actor) {
    ...
    if (hands.off === 'shield' && ['sword', 'dagger', 'spear', 'mace', 'axe'].includes(hands.main))
  ...
  U.CharacterAdapter = {
    getState(actor, options = {}) {
      ...
      const base = exalted
        ...
        : {
            ...source,
            // FIX: los aldeanos (vendedores, etc.) usaban siempre el
            // mismo color de ropa fijo por tipo — todos los vendedores
            // se veían idénticos. Si el NPC trae su propia variante de
            // ropa asignada (ver world-services.js), se usa esa en vez
            // del color base del tipo.
            shirt: actor.outfitShirt || source.shirt,
            pants: actor.outfitPants || source.pants,
            tunic: actor.outfitTunic || source.tunic,
            hair: actor.outfitHair || source.hair,
            armor: equippedColor(actor, 'armor', source.armor),
            weapon: equippedColor(actor, 'weapon', source.armor),
          };
      /* Color por pieza: cada parte del cuerpo respeta el material que
         tiene puesto (mithril en el pecho se ve morado, hierro en los
         guantes se ve gris, al mismo tiempo). base.armor (arriba) se deja
         como respaldo general para cualquier parte sin pieza propia. */
      if (!exalted) {
        base.armorParts = {
          chest: slotColor(actor, 'chest', base.armor),
          arms: slotColor(actor, 'arms', base.armor),
          gloves: slotColor(actor, 'gloves', base.armor),
          pants: slotColor(actor, 'pants', base.armor),
          boots: slotColor(actor, 'boots', base.armor),
          head: slotColor(actor, 'head', base.armor),
          neck: slotColor(actor, 'neck', base.armor),
        };
      }
      return {
        base,
        ...
        /* FIX: esto exigía ser el jugador para siquiera mirar el equipo
           real (actor.equipment). Cualquier otro personaje —aunque
           tuviera equipo real de verdad, como ahora los guardias— caía
           siempre al valor fijo de la derecha. Ahora cualquier actor con
           un objeto de equipo real se trata igual que el jugador. */
        showClothes:
          !(player || actor.equipment) ||
          ['head', 'neck', 'chest', 'arms', 'gloves', 'pants', 'boots'].some(
            slot => layer(actor, slot) === 'clothing',
          ),
        clothes: player || actor.equipment
          ? {
        ...
        /* FIX RAÍZ: acá estaba el bug real de "los guardias no llevan
           armadura". Sus piezas de hierro se equipan bien (equipment sí
           las tiene), pero esta condición exigía "ser el jugador" para
           siquiera consultarlas — para cualquier otro personaje caía
           directo al campo viejo actor.armor (un string tipo 'plate'),
           que los guardias nunca tuvieron. El equipo real existía, pero
           el renderer jamás lo miraba. */
        equip: {
          helmet: ((player || actor.equipment) && layer(actor, 'head') === 'armor') || ['hood', 'plate'].includes(actor.armor),
          neck: (player || actor.equipment) && layer(actor, 'neck') === 'armor',
          torso: ((player || actor.equipment) && layer(actor, 'chest') === 'armor') || ['mail', 'plate'].includes(actor.armor),
          arms: ((player || actor.equipment) && layer(actor, 'arms') === 'armor') || ['bracers', 'mail', 'plate'].includes(actor.armor),
          gloves: (player || actor.equipment) && layer(actor, 'gloves') === 'armor',
          legs: ((player || actor.equipment) && layer(actor, 'pants') === 'armor') || actor.armor === 'plate',
          boots: ((player || actor.equipment) && layer(actor, 'boots') === 'armor') || actor.armor === 'plate',
        },
        cape: (player || actor.equipment) && equipped(actor, 'cloak'),
        tunic: (player || actor.equipment) && equipped(actor, 'robe'),
