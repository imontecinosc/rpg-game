(function (U) {
  U.update = function (dt) {
    ...
    const move = p.speed * (1 + (b.moveSpeed || 0) / 100) * (p.casting ? U.CAST_SLOW : 1);
    p.vx = dx * move;
    p.vy = dy * move;
    ...
    if (p.bandaging) {
      ...
      if (p.bandaging.t <= 0) {
        if (p.bandaging.gather === 'mine') {
          const mining = p.skills.Minería || 0,
            roll = Math.random(),
            ore = mining >= 60 && roll < 0.12 ? 'goldOre' : roll < 0.38 ? 'copperOre' : 'ironOre';
          U.addItem(ore, Math.round(U.rnd(4, 8)));
          U.raiseSkill('Minería', 0.25);
        } else if (p.bandaging.gather === 'wood') {
          const lumber = p.skills.Tala || 0,
            roll = Math.random(),
            wood =
              lumber >= 80 && roll < 0.08
                ? 'ironwood'
                : lumber >= 60 && roll < 0.2
                  ...
                  : lumber >= 40 && roll < 0.4
                    ? 'oakWood'
                    : 'wood';
          U.addItem(wood, Math.round(U.rnd(4, 8)));
          U.raiseSkill('Tala', 0.25);
        } else if (p.bandaging.gather === 'fish') {
          U.addItem('fish', 1);
          U.raiseSkill('Pesca', 0.25);
        } else {
          p.hp = Math.min(p.maxHp, p.hp + 28 + p.skills.Curar * 0.12);
          p.healCd = 2.5;
          U.toast('Terminas de vendarte.');
        }
        p.bandaging = null;
        U.ui.refreshAll();
      }
    ...
    for (const e of U.enemies) {
      ...
      if (e.aggro && playerDist < 1.3 && e.attackCd <= 0) {
        ...
        U.state.effects.push({
          ...
          x: p.x,
          y: p.y,
          text: blocked ? 'BLOQUEO' : '-' + taken,
          color: blocked ? '#9ed6ff' : '#ff9a91',
          life: 1,
        });
        if (p.hp <= 0) U.die();
      }
    ...
    for (let i = U.state.projectiles.length - 1; i >= 0; i--) {
      ...
      if (d < q + 0.25) {
        ...
        if (pr.spell)
          dmg =
            18 + U.player.skills.Magia * 0.11 + (pr.spell.id === 'lightning' ? 9 : 0) + (b.magicPower || 0);
  ...
  U.respawn = function () {
    ...
    p.target = null;
    U.state.autoAttack = false;
    U.toast('Reapareces en ' + p.respawn.name);
    U.ui.refreshAll();
  };
  U.save = function () {
