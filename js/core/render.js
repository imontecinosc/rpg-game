(function (U) {
  ...
  // Esta capa pertenece al viewport, no al mundo. El controlador de cámara la
  // invoca después de restaurar el zoom para que siempre cubra toda la pantalla.
  U.dibujarVinietaPantalla = function () {
    const c = U.ctx;
    c.save();
  ...
  function hpBar(o, p) {
    if ((U.cameraZoom || 1) < 0.68 && o !== U.player.target && !o.boss && !o.variant) return;
    const c = U.ctx,
  ...
  function playerBars(o, p) {
    ...
    [
      [o.hp, o.maxHp, '#b9463f'],
      [o.mana, o.maxMana, '#477dcc'],
      [o.stam, o.maxStam, '#c8a943'],
    ].forEach(([value, max, color], i) => {
      c.fillStyle = 'rgba(12,10,9,.88)';
      c.fillRect(p.x - w / 2, y + i * 5, w, h);
      c.fillStyle = color;
      c.fillRect(p.x - w / 2 + 1, y + i * 5 + 1, (w - 2) * U.clamp(value / max, 0, 1), 1);
    });
  }
  function label(o, p, isPlayer = false) {
    if (!isPlayer && (U.cameraZoom || 1) < 0.68 && o !== U.player.target && !o.boss && !o.variant) return;
    const c = U.ctx,
      name = isPlayer ? U.player.name : o.name || '';
    ...
    c.lineWidth = 3;
    const nameY =
      p.y - (isPlayer ? 87 : o.boss ? 96 : o.variant === 'exalted' ? 88 : o.variant === 'renowned' ? 81 : 77);
    c.strokeText(name, p.x, nameY);
    c.fillText(name, p.x, nameY);
    c.textAlign = 'start';
    if (isPlayer) playerBars(o, p);
  }
  ...
  function humanoid(o, isPlayer = false) {
    ...
    if (U.CharacterRenderer && U.CharacterAdapter) {
      if (isPlayer) {
        ...
        c.lineWidth = 3;
        c.beginPath();
        c.ellipse(0, 38, 23, 10, 0, 0, Math.PI * 2);
        c.stroke();
      }
  ...
  U.draw = function () {
    ...
    const zoom = U.cameraZoom || 1,
      span = Math.ceil(Math.max(U.W / (ISO_X * 2), U.H / (ISO_Y * 2)) / zoom) + 7;
    ...
    for (const city of U.cities) {
      ...
      // Muralla perimetral con cuatro accesos amplios y siempre transitables.
      const wallR = city.safe - 1;
      for (let i = -wallR; i <= wallR; i += 2) {
        if (Math.abs(i) < 5) continue;
        for (const [wx, wy] of [
          [city.x + i, city.y - wallR],
          [city.x + i, city.y + wallR],
          [city.x - wallR, city.y + i],
          [city.x + wallR, city.y + i],
        ]) {
          const wp = U.worldToScreen(wx, wy);
          c.fillStyle = tono(city.color, -28);
          c.fillRect(wp.x - 6, wp.y - 19, 12, 20);
          c.fillStyle = tono(city.color, 8);
          c.fillRect(wp.x - 7, wp.y - 22, 14, 5);
        }
      }
    }
    // Vegetación y piedra procedural: decoración determinista sin colisión.
    for (let y = Math.floor(U.player.y) - 28; y < Math.floor(U.player.y) + 28; y += 2)
      for (let x = Math.floor(U.player.x) - 28; x < Math.floor(U.player.x) + 28; x += 2) {
        const n = ruido(x * 3, y * 5);
        const inCity = U.cities.some(z => Math.hypot(x - z.x, y - z.y) < z.safe + 2);
        const mine = U.terrain.mines.find(m => Math.hypot(x - m.x, y - m.y) < m.r);
        const forest = U.terrain.forests.find(f => Math.hypot(x - f.x, y - f.y) < f.r);
        const p = U.worldToScreen(x, y);
        if (mine && n > 0.72) {
          c.fillStyle = n > 0.87 ? '#8b8780' : '#696660';
          c.beginPath();
          c.moveTo(p.x - 7, p.y);
          c.lineTo(p.x - 2, p.y - 10);
          c.lineTo(p.x + 7, p.y - 3);
          c.lineTo(p.x + 5, p.y + 2);
          c.closePath();
          c.fill();
        } else if (!inCity && forest && n > 0.76) {
          c.fillStyle = '#3e2d21';
          c.fillRect(p.x - 2, p.y - 22, 4, 21);
          c.fillStyle = n > 0.88 ? '#315f39' : '#294f31';
          c.beginPath();
          c.arc(p.x, p.y - 27, 12, 0, Math.PI * 2);
          c.fill();
        } else if (!inCity && !mine && n > 0.62) {
          c.strokeStyle = '#4f7547';
          c.lineWidth = 1;
          c.beginPath();
          c.moveTo(p.x, p.y);
          c.lineTo(p.x - 3, p.y - 7);
          c.moveTo(p.x, p.y);
          c.lineTo(p.x + 4, p.y - 6);
          c.stroke();
        }
      }
    ...
    for (const o of objects) {
      ...
      if (
        ...
        o.aggro &&
        !o.dead &&
        U.dist(o, U.player) < 1.65 &&
        o.attackCd > 0 &&
        o.attackCd < 0.28
      ) {
        c.save();
        c.strokeStyle = `rgba(255,190,74,${0.45 + (0.28 - o.attackCd) * 1.8})`;
        c.lineWidth = 3;
        c.beginPath();
        c.ellipse(shadow.x, shadow.y + 12, 24, 10, 0, 0, Math.PI * 2);
        c.stroke();
        c.fillStyle = '#ffd36c';
        c.font = 'bold 15px Arial';
        c.textAlign = 'center';
        c.fillText('!', shadow.x, shadow.y - 70);
        c.restore();
      }
      c.save();
    ...
    if (U.player.target && !U.player.target.dead) {
      ...
      const targetIsHumanoid = [
        ...
        'zombie',
        'skeleton',
      ].includes(U.player.target.type);
      c.beginPath();
      c.ellipse(p.x, p.y + (targetIsHumanoid ? 32 : 12), 25, 11, 0, 0, Math.PI * 2);
      c.stroke();
      c.fillStyle = '#ff655d';
      c.beginPath();
      c.moveTo(p.x, p.y - 76);
