...
function weaponAngle(dir,id){
  ...
  const S=SET();
  return{limb:eqOn('arms')?S.limb(base,front):(clothOn('arms')?shade(base.shirt,front?8:-12):shade(base.skin,front?6:-6)),
         hand:eqOn('gloves')?S.hand(base,front):(clothOn('gloves')?shade(base.shirt,-18):shade(base.skin,front?10:-12))};
}
  ...
  const col=eqOn('legs')?(front?shade(base.armor,-2):shade(base.armor,-16)):(clothOn('pants')?cloth:skin);
  ...
  if(eqOn('legs')) SET().legPlate(hip,knee,foot,front,base);
  if(eqOn('boots')) SET().boot(knee,foot,base);
  const sole=eqOn('boots')?SET().sole(base):'#2e241d';
...
function drawSideLeg(front,base){
  ...
  ell(foot.x+(running?5:3),foot.y+7,running?13.4:12.5,running?4.5:4.8,
    eqOn('boots')?SET().sole(base):'#2e241d',-0.055*swing);
}
