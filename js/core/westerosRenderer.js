 ...
 */  
 ...
 pickaxe: { name: 'Pico de minería', hands: 2, kind: 'tool', reach: 108, arc: 'work' },  
 workAxe: { name: 'Hacha de tala', hands: 2, kind: 'tool', reach: 104, arc: 'work' },  
 };  
 const OFF = {  
 ...
 run: { dur: 0.58, loop: true },  
 mine: { dur: 1.18, loop: true },  
 chop: { dur: 1.08, loop: true },  
 ...
 let act=state.tab==='humans'?state.action:state.beastAction;  
 // La barra de bestias ofrece 'attack' pero el clip se llama 'melee'.  
 // Sin esta linea la bestia caia a idle y no atacaba nunca.  
 if(act==='attack')act='melee';  
 ...
 De perfil el braceo y el balanceo se acentúan; de frente/espaldas  
 se atenúan porque ahí la profundidad la resuelve la pierna, no el torso. */  
 const ph=t*Math.PI*2;  
 const sideView=state.tab==='humans'&&(state.dir==='left'||state.dir==='right');  
 P.legPhase = Math.sin(ph);  
 P.armPhase = -Math.sin(ph)*(sideView?1.65:1.42);  
 P.bob = sideView ? Math.abs(Math.cos(ph))*3.8-1.9 : Math.abs(Math.cos(ph))*4.6-2.3;  
 P.torsoTwist = Math.sin(ph)*(sideView?0.045:0.095);  
 P.torsoLean = sideView ? 0.075+Math.sin(ph)*0.008 : 0.045+Math.cos(ph*2)*0.012;  
 P.headTilt = -Math.sin(ph)*(sideView?0.018:0.022);  
 P.headY = Math.abs(Math.cos(ph))*(sideView?0.9:1.35);  
 ...
 /* Carrera: zancada amplia, impulso hacia delante y suspensión breve. */  
 const ph=t*Math.PI*2;  
 const sideView=state.tab==='humans'&&(state.dir==='left'||state.dir==='right');  
 const flight=Math.pow(Math.max(0,-Math.cos(ph*2)),1.35);  
 P.legPhase = Math.sin(ph);  
 P.armPhase = -Math.sin(ph)*(sideView?2.45:2.05);  
 P.bob = Math.abs(Math.cos(ph))*(sideView?6.2:7.2)-3.1-flight*2.2;  
 P.torsoTwist = Math.sin(ph)*(sideView?0.075:0.145);  
 P.torsoLean = sideView ? 0.19+Math.cos(ph*2)*0.018 : 0.105+Math.cos(ph*2)*0.02;  
 P.headTilt = -Math.sin(ph)*(sideView?0.03:0.035);  
 P.headY = Math.abs(Math.cos(ph))*(sideView?1.5:2.0);  
 P.breath = Math.sin(ph);  
 P.guard = 0.08;  
 P.capeSway = Math.sin(ph-0.75)*12;  
 P.capeLift = 0.62+Math.abs(Math.sin(ph))*0.28+flight*0.12;  
 }  
  
 else if(act==='mine'||act==='chop'){  
 /* Trabajo a dos manos: preparar → acelerar → impacto → recuperar.  
 Minar cae más vertical y bajo; talar recorre un arco más lateral. */  
 const mining=act==='mine';  
 if(t<0.34){  
 const k=eOutCubic(seg01(t,0,0.34));  
 P.workLift=k; P.workDrive=0;  
 P.armPhase=1.8*k;  
 P.torsoLean=-0.06*k;  
 P.torsoTwist=(mining?-0.08:-0.18)*k;  
 P.sink=2.5*k;  
 } else if(t<0.57){  
 const k=eInCubic(seg01(t,0.34,0.57));  
 P.workLift=1-k; P.workDrive=k;  
 P.workImpact=Math.pow(k,3);  
 P.armPhase=1.8-3.2*k;  
 P.torsoLean=-0.06+(mining?0.34:0.24)*k;  
 P.torsoTwist=(mining?-0.08:-0.18)+(mining?0.18:0.48)*k;  
 P.sink=2.5+(mining?10:7)*k;  
 P.bob=3.5*k;  
 } else if(t<0.69){  
 const k=seg01(t,0.57,0.69);  
 P.workDrive=1-k*0.18;  
 P.workImpact=1-k;  
 P.armPhase=-1.4+0.22*k;  
 P.torsoLean=(mining?0.28:0.18)*(1-k*0.18);  
 P.torsoTwist=(mining?0.1:0.3)*(1-k*0.15);  
 P.sink=(mining?12.5:9.5)-k*1.5;  
 P.bob=3.5-k;  
 } else {  
 const k=eOutCubic(seg01(t,0.69,1));  
 P.workDrive=0.82*(1-k);  
 P.armPhase=-1.18*(1-k);  
 P.torsoLean=(mining?0.23:0.15)*(1-k);  
 P.torsoTwist=(mining?0.085:0.255)*(1-k);  
 P.sink=(mining?11:8)*(1-k);  
 P.bob=2.5*(1-k);  
 }  
 P.legPhase = mining ? -0.24*P.workDrive : -0.14*P.workDrive;  
 P.headTilt = -0.08*P.workDrive;  
 P.capeSway = (mining?5:9)*(P.workLift-P.workDrive);  
 P.capeLift = 0.16+P.workLift*0.22;  
 }  
 ...
 function studs(pts,c,r=1.9){ctx.fillStyle=c;pts.forEach(([x,y])=>{ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()})}  
 function escamaFila(y,ancho,off,c,paso=6.8,alto=8.5){  
 for(let x=-ancho+off;x<=ancho-3;x+=paso){  
 ctx.beginPath();ctx.moveTo(x-3.2,y);ctx.quadraticCurveTo(x,y+alto,x+3.2,y);ctx.closePath();  
 ctx.fillStyle=c;ctx.fill();ctx.strokeStyle=O;ctx.lineWidth=.8;ctx.stroke();  
 }  
}  
 ...
 ctx.strokeStyle=c;ctx.lineWidth=grosor;  
 for(let i=1;i<=n;i++){  
 const t=i/(n+1), px=a.x+dx*t, py=a.y+dy*t;  
 ctx.beginPath();ctx.moveTo(px-nx*ancho,py-ny*ancho);ctx.lineTo(px+nx*ancho,py+ny*ancho);ctx.stroke();  
 }  
}  
 function tachasHueso(a,b,n,c,r=1.8){  
 const pts=[];  
 for(let i=1;i<=n;i++){const t=i/(n+1);pts.push([a.x+(b.x-a.x)*t,a.y+(b.y-a.y)*t])}  
 studs(pts,c,r);  
}  
 /* Casco: corona común por dirección; cada set le agrega lo suyo. */  
 function coronaCasco(dir,c,alto=0,ancho=0){  
 const top=RIG.headTop,cy=RIG.headCy,w=16+ancho,t=top-7-alto;  
 if(dir==='down') poly([[-w,cy+2],[-w,top+2],[-9,t],[9,t],[w,top+2],[w,cy+2],[10,cy-3],[-10,cy-3]],c,O,1.5);  
 else if(dir==='up') poly([[-w,cy+6],[-w,top+2],[-9,t],[9,t],[w,top+2],[w,cy+6],[0,cy+10]],c,O,1.5);  
 else poly([[-w-1,cy+2],[-w,top+1],[-6,t-1],[9,top-3],[w-1,cy-1],[9,cy-4],[-11,cy-4]],c,O,1.5);  
}  
 ...
 function cuero(x1,y1,x2,y2,w,c){ seg(x1,y1,x2,y2,w,c,'butt') }  
 function cordel(y0,y1,c,paso=3.4){  
 ctx.strokeStyle=c;ctx.lineWidth=1.5;  
 for(let y=y0;y<y1;y+=paso){ctx.beginPath();ctx.moveTo(-3.4,y);ctx.lineTo(3.4,y+1.6);ctx.stroke()}  
}  
 function estrias(y0,y1,c,paso=4){  
 ctx.strokeStyle=c;ctx.lineWidth=1.8;  
 for(let y=y0;y<y1;y+=paso){ctx.beginPath();ctx.moveTo(-3.2,y);ctx.lineTo(3.2,y);ctx.stroke()}  
}  
 ...
 for(let i=0;i<=10;i++){  
 const t=i/10, y=-largo*t, x=Math.sin(t*7.2)*ancho*0.34;  
 ...
 ctx.beginPath();ctx.moveTo(ancho/2,y);ctx.lineTo(ancho/2+2.6,y-2.2);ctx.lineTo(ancho/2,y-4.4);  
 ...
 if(id==='pickaxe'||id==='workAxe'){  
 ctx.save(); ctx.translate(hx,hy); ctx.rotate(angle); drawWorkTool(id); ctx.restore();  
 return;  
 }  
 const fam=ARMA();  
 const dibujo=fam[id]||ART[id];  
 if(!dibujo)return;  
 ctx.save(); ctx.translate(hx,hy); ctx.rotate(angle); dibujo.call(fam); ctx.restore();  
}  
  
 /* ══════════════════════════════════════════════════════════════════  
 6 · ÁNGULOS DE ARMA POR DIRECCIÓN Y CLIP  
 ══════════════════════════════════════════════════════════════════ */  
 function drawWorkTool(id){  
 seg(0,14,0,-94,7,'#68462b');  
 seg(-1.5,10,-1.5,-91,2,'#a77645');  
 if(id==='pickaxe'){  
 poly([[-54,-99],[-8,-108],[0,-101],[-8,-94]],'#aab1b5','#3a4145',2);  
 poly([[54,-99],[8,-108],[0,-101],[8,-94]],'#91999e','#3a4145',2);  
 ell(0,-101,7,9,'#626b70');  
 } else {  
 poly([[-5,-111],[36,-104],[48,-88],[29,-72],[-7,-80]],'#9da5aa','#343b40',2);  
 seg(29,-101,42,-88,2,'#d4d9dc');  
 ell(0,-94,7,10,'#596166');  
 }  
}  
...
function weaponAngle(dir,id){  
 ...
 if(m.arc==='work'){  
 const side=dir==='side';  
 const mining=id==='pickaxe';  
 const raised=side?-0.92:-0.36;  
 const impact=side?(mining?1.02:1.34):(mining?0.92:1.18);  
 const a=raised*P.workLift+impact*P.workDrive+0.18*(1-P.workLift-P.workDrive);  
 return dir==='up' ? Math.PI-a : a;  
 }  
 ...
 ctx.beginPath();ctx.moveTo(4,RIG.neckTop+2);ctx.lineTo(7,RIG.neckBot);ctx.stroke();  
 if(eqOn('neck')){  
 SET().neck({...base,armor:base.armorParts?.neck??base.armor});  
 ...
 SET().torso({...base,armor:base.armorParts?.chest??base.armor});  
}  
 function drawPauldron(sx,sy,side,base){  
 if(!eqOn('torso'))return;  
 const s2=side==='left'?-1:1;  
 ctx.save();ctx.translate(sx,sy);  
 SET().pauldron(s2,{...base,armor:base.armorParts?.arms??base.armor});  
 ctx.restore();  
}  
 function drawHelmet(dir,base){  
 if(!eqOn('helmet'))return;  
 ctx.save();  
 SET().helmet(dir,{...base,armor:base.armorParts?.head??base.armor});  
 ctx.restore();  
}  
 ...
 const S=SET();  
 const armBase={...base,armor:base.armorParts?.arms??base.armor};  
 const gloveBase={...base,armor:base.armorParts?.gloves??base.armor};  
 return{limb:eqOn('arms')?S.limb(armBase,front):(clothOn('arms')?shade(base.shirt,front?8:-12):shade(base.skin,front?6:-6)),  
 hand:eqOn('gloves')?S.hand(gloveBase,front):(clothOn('gloves')?shade(base.shirt,-18):shade(base.skin,front?10:-12))};  
}  
 ...
 if(eqOn('arms')) SET().armPlate(shoulder,elbow,hand,front,{...base,armor:base.armorParts?.arms??base.armor});  
 if(eqOn('gloves')) SET().glove(hand,front,{...base,armor:base.armorParts?.gloves??base.armor});  
 ...
 const up=dir==='up';  
 const working=state.action==='mine'||state.action==='chop';  
 if(working){  
 const mining=state.action==='mine';  
 const lift=P.workLift, drive=P.workDrive;  
 const sh={x:s*RIG.shoulderX,y:RIG.shoulderY+8};  
 const lateralBias=mining?0:(dir==='up'?-1:1)*7*(lift+drive);  
 const handX=s*(weaponArm?13:7)+lateralBias+(weaponArm?5:-4)*drive;  
 const handY=(weaponArm?7:-8)-lift*(weaponArm?91:76)+drive*(weaponArm?47:34);  
 const hd={x:handX,y:handY};  
 const el={x:s*(RIG.shoulderX+11)+lateralBias*0.55+(weaponArm?-5:6)*lift,  
 y:RIG.elbowY-lift*39+drive*18};  
 return{sh,el,hd};  
 }  
 ...
 if((state.action==='walk'||state.action==='run')&&(dir==='down'||dir==='up')){  
 drawAxialLeg(xF,front,base,dir);  
 return;  
 }  
 ...
 const legArmor=base.armorParts?.pants??base.armor;  
 const col=eqOn('legs')?(front?shade(legArmor,-2):shade(legArmor,-16)):(clothOn('pants')?cl:skin);  
 ...
 if(eqOn('legs')) SET().legPlate(hip,knee,foot,front,{...base,armor:legArmor});  
 if(eqOn('boots')) SET().boot(knee,foot,{...base,armor:base.armorParts?.boots??base.armor});  
 ell(foot.x,foot.y+7,11.5,5,eqOn('boots')?SET().sole({...base,armor:base.armorParts?.boots??base.armor}):'#2e241d');  
}  
 function drawAxialLeg(xF,front,base,dir){  
 const running=state.action==='run';  
 const cycle=(clipT/CLIPS[state.action].dur)*Math.PI*2+(front?0:Math.PI);  
 const depth=Math.sin(cycle);  
 const swingLift=Math.max(0,Math.cos(cycle))*(running?11.5:6.5);  
 const flight=running?Math.pow(Math.max(0,-Math.cos(cycle*2)),1.4)*2.4:0;  
 const travelSign=dir==='down'?1:-1;  
 const depthTravel=depth*(running?14.5:8.5)*travelSign;  
 const outward=(front?1:-1)*((running?2.0:1.6)+Math.max(0,depth)*(running?1.6:1.2));  
 const skin=front?shade(base.skin,6):shade(base.skin,-8);  
 const cloth=front?shade(base.pants,10):shade(base.pants,-12);  
 const legArmor=base.armorParts?.pants??base.armor;  
 const col=eqOn('legs')?(front?shade(legArmor,-2):shade(legArmor,-16)):(clothOn('pants')?cloth:skin);  
 const hip={x:xF+outward*0.18,y:RIG.hipY+P.sink*0.25};  
 const knee={x:xF+outward+depth*(front?1.8:-1.8),  
 y:RIG.kneeY+depthTravel*0.48-swingLift*0.72-flight+P.sink*0.5};  
 const foot={x:xF+outward*1.35+depth*(front?2.4:-2.4),  
 y:RIG.footY+depthTravel-swingLift-flight+P.sink*0.2};  
 seg(hip.x,hip.y,knee.x,knee.y,13,col);  
 seg(knee.x,knee.y,foot.x,foot.y,10,col);  
 if(eqOn('legs')) SET().legPlate(hip,knee,foot,front,{...base,armor:legArmor});  
 const bootBase={...base,armor:base.armorParts?.boots??base.armor};  
 if(eqOn('boots')) SET().boot(knee,foot,bootBase);  
 const sole=eqOn('boots')?SET().sole(bootBase):'#2e241d';  
 const planted=1-swingLift/(running?11.5:6.5);  
 ell(foot.x,foot.y+7,11.5+planted*1.4,4.2+planted*0.8,sole,depth*0.025);  
}  
  
function drawSideLeg(front,base){  
 const running=state.action==='run';  
 const hipDrop=running?3.5:0;  
 const walkPhase=(clipT/CLIPS[state.action].dur)*Math.PI*2+(front?0:Math.PI);  
 const swing=Math.sin(walkPhase);  
 const advancing=Math.max(0,Math.cos(walkPhase));  
 const flight=running?Math.pow(Math.max(0,-Math.cos(walkPhase*2)),1.35)*2.8:0;  
 const stride=swing*(running?34:22);  
 const lift=advancing*(running?14:9);  
 const hipX=front?-4:4;  
 const skin=front?shade(base.skin,6):shade(base.skin,-8);  
 const cloth=front?shade(base.pants,10):shade(base.pants,-12);  
 const legArmor=base.armorParts?.pants??base.armor;  
 const col=eqOn('legs')?(front?shade(legArmor,-2):shade(legArmor,-16)):(clothOn('pants')?cloth:skin);  
 const hip={x:hipX,y:RIG.hipY+hipDrop+P.sink*0.25};  
 const knee={x:hipX+stride*0.48+advancing*4,  
 y:RIG.kneeY+hipDrop*0.45-lift*(running?0.55:0.42)-flight+P.sink*0.5};  
 const foot={x:hipX+stride,y:RIG.footY-lift-flight+P.sink*0.2};  
 seg(hip.x,hip.y,knee.x,knee.y,13,col);  
 seg(knee.x,knee.y,foot.x,foot.y,10,col);  
 if(eqOn('legs')) SET().legPlate(hip,knee,foot,front,{...base,armor:legArmor});  
 const bootBase={...base,armor:base.armorParts?.boots??base.armor};  
 if(eqOn('boots')) SET().boot(knee,foot,bootBase);  
 ell(foot.x+(running?5:3),foot.y+7,running?13.4:12.5,running?4.5:4.8,  
 eqOn('boots')?SET().sole(bootBase):'#2e241d',-0.055*swing);  
}  
...
function drawTunic(base,dir){  
 ...
 if(state.action==='mine') return 'pickaxe';  
 if(state.action==='chop') return 'workAxe';  
 ...
 const workInFront=m==='pickaxe'||m==='workAxe';  
 if(m&&!workInFront){  
 ...
 drawHead('down',base);  
 /* El pico/hacha cruza entre la cámara y el personaje al golpear hacia  
 abajo: debe quedar como el último estrato, sobre la cabeza incluida. */  
 if(workInFront){  
 const ang=weaponAngle('down',m);  
 drawWeaponAt(m,R.hd.x,R.hd.y,ang);  
 }  
}  
 ...
 const m=mainHandItem();  
 const workBehindBody=m==='pickaxe'||m==='workAxe';  
 /* En la vista posterior, la herramienta de trabajo debe quedar en el  
 estrato más profundo: el personaje está entre la cámara y el útil. */  
 if(workBehindBody){  
 const ang=weaponAngle('up',m);  
 drawWeaponAt(m,R.hd.x,R.hd.y,ang);  
 }  
 /* Escudo y brazo secundario también parten al fondo. */  
 ...
 const sideWalk=state.action==='walk'||state.action==='run';  
 const sideWalking=state.action==='walk';  
 const sideRun=state.action==='run';  
 const sideWork=state.action==='mine'||state.action==='chop';  
 /* La carrera lateral articula el bloque superior desde la pelvis.  
 En coordenadas locales el personaje siempre mira a la derecha; el  
 espejo de la vista izquierda invierte automáticamente la inclinación. */  
 const drawUpper=(draw,worldSpace=false)=>{  
 ctx.save();  
 if(sideRun){  
 const hipDrop=3.5;  
 const chestLean=0.105+Math.cos((clipT/CLIPS.run.dur)*Math.PI*4)*0.008;  
 ctx.translate(0,RIG.hipY+hipDrop);  
 ctx.rotate(chestLean*(worldSpace&&flip?-1:1));  
 ...
 drawUpper(()=>drawCape(base,dir));  
 drawUpper(()=>longHair(base,'right'));  
 /* Escudo al fondo en ambos perfiles. */  
 if(hasShield())drawUpper(()=>drawWeaponAt('shield',16,-14,0));  
 /* brazo trasero: en perfil, los hombros acompañan el braceo en oposición.  
 La carrera amplía el recorrido, pero conserva el mismo principio que  
 la marcha. */  
 const rc=armColors(base,false);  
 const rs=P.armPhase*-1;  
 const shoulderTravel=sideWalking?2.8:sideRun?4.4:0;  
 const rSh={x:2+rs*shoulderTravel,y:RIG.shoulderY+8+(sideWalk?Math.abs(rs)*0.45:0)};  
 const elbowFold=sideWalking?5.5:sideRun?7.2:0;  
 const rElX=sideWork?10-P.workLift*12+P.workDrive*15:sideWalk?10+rs*5.2-elbowFold:15;  
 const rElY=sideWork?RIG.elbowY-P.workLift*42+P.workDrive*15:sideWalk?RIG.elbowY+Math.abs(rs)*1.4:RIG.elbowY+rs*2.2;  
 const rHdX=sideWork?6-P.workLift*18+P.workDrive*29:sideWalk?15+rs*10.5:22;  
 const rHdY=sideWork?-8-P.workLift*76+P.workDrive*34:sideWalk?RIG.handY+Math.abs(rs)*2.2:RIG.handY+rs*3.4;  
 const rEl={x:rElX,y:rElY}, rHd={x:rHdX,y:rHdY};  
 drawUpper(()=>{  
 seg(rSh.x,rSh.y,rEl.x,rEl.y,10,rc.limb);  
 seg(rEl.x,rEl.y,rHd.x,rHd.y,9,rc.hand);  
 if(eqOn('arms')) SET().armPlate(rSh,rEl,rHd,false,base);  
 if(eqOn('gloves')) SET().glove(rHd,false,base);  
 else ell(rHd.x,rHd.y,5,5,rc.hand);  
 if(isDualWield())drawWeaponAt(state.off,rHd.x,rHd.y,-0.86-P.swing*0.4);  
 });  
 /* pierna trasera */  
 if(sideWalk) drawSideLeg(false,base);  
 else drawLeg(4,false,base,dir);  
 drawUpper(()=>{  
 drawBody(base,true);  
 drawPauldron(rSh.x,rSh.y-4,'right',base);  
 });  
 /* pierna delantera */  
 if(sideWalk) drawSideLeg(true,base);  
 else drawLeg(-6,true,base,dir);  
 drawUpper(()=>{  
 drawNeck(base);  
 ...
 const fc=armColors(base,true);  
 const fs=P.armPhase, cast=P.castCharge;  
 const sh={x:-3+fs*shoulderTravel,y:RIG.shoulderY+8+(sideWalk?Math.abs(fs)*0.45:0)};  
 const elX=sideWork?-4-P.workLift*17+P.workDrive*21:sideWalk?4+fs*5.2-elbowFold:-3+fs*3.2;  
 const elY=sideWork?RIG.elbowY-P.workLift*47+P.workDrive*19:sideWalk?RIG.elbowY+Math.abs(fs)*1.4:RIG.elbowY+fs*2.4-cast*20;  
 const hdX=sideWork?13-P.workLift*22+P.workDrive*38:sideWalk?10+fs*10.5:14+fs*6.4;  
 const hdY=sideWork?7-P.workLift*91+P.workDrive*47:sideWalk?RIG.handY+Math.abs(fs)*2.2:RIG.handY+fs*4.2-cast*44;  
 const el={x:elX,y:elY}, hd={x:hdX,y:hdY};  
 drawUpper(()=>{  
 seg(sh.x,sh.y,el.x,el.y,10,fc.limb);  
 seg(el.x,el.y,hd.x,hd.y,9,fc.hand);  
 if(eqOn('arms')) SET().armPlate(sh,el,hd,true,base);  
 if(eqOn('gloves')) SET().glove(hd,true,base);  
 else ell(hd.x,hd.y,5.2,5.2,fc.hand);  
 if(hasSpellbook()&&!sideWork)drawWeaponAt('spellbook',hd.x,hd.y,0.1);  
 const m=mainHandItem();  
 if(m){  
 ...
 if(hasSpellbook()&&!m&&!sideWork)fxMagic(hd.x,hd.y);  
 });  
 ctx.restore();  
 drawUpper(()=>drawHead(dir,base),true);  
}  
 ...
 const lateralRun=state.action==='run'&&(state.dir==='left'||state.dir==='right');  
 /* En perfil la inclinación de carrera se resuelve dentro de composeSide  
 desde la pelvis; aquí se evita volver a rotar el personaje completo. */  
 ctx.rotate(P.fall*(Math.PI/2)*0.92 + (lateralRun?0:P.torsoLean*0.35) + P.recoil*0.05);  
