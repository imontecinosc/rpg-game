(function (U) {
  ...
  function targetCandidates() {
    return [
      ...
      .filter(object => U.dist(U.player, object) <= 12)
      .sort((a, b) => U.dist(U.player, a) - U.dist(U.player, b));
  }

  U.cycleTarget = function () {
    const candidates = targetCandidates();
    if (!candidates.length) return U.toast('No hay objetivos cercanos.');
    const index = candidates.indexOf(U.player.target);
    U.player.target = candidates[(index + 1) % candidates.length];
    const hostile = U.enemies.includes(U.player.target);
    U.state.autoAttack = hostile && U.state.battle;
    U.ui.updateBattleUI();
