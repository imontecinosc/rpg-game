(function (U) {
    ...
    U.npcs.push(
      {
        ...
        nameColor: '#f2cf75',
        facing: 'left',
        outfitShirt: '#8f6f48', outfitPants: '#4a3a28', outfitTunic: '#a2865e', outfitHair: '#4a3626',
      },
  ...
  /* FIX: el botón "Usar" y la tecla E llaman a U.useAction (definida en
     world-services.js), no a un "U.useContext" que nunca existió en el
     resto del código. Esa función fantasma quedaba declarada pero jamás
     invocada, así que el carro y los NPC de misión no respondían al
     interactuar por botón/tecla — solo funcionaban si se los tocaba
     directamente en pantalla, porque ese camino sí pasa por interactNpc. */
  const oldUseAction = U.useAction;
  ...
  U.killEnemy = function (enemy) {
    ...
    if (enemy.questRole === 'captain') {
      ...
      /* advance() no hace nada si stage ya llegó a 5 (pasa si el jugador
         habló con Dagan antes de matar al capitán), así que el aviso de
         "andá a buscar a Dagan" se manda siempre, aparte de advance(). */
      if (state.stage >= 4) advance(5, null);
      U.toast('El capitán ha caído. Vuelve con el capataz Dagan para recuperar el Corazón.');
    } else {
      state.campKills++;
  ...
  /* Pedido explícito: se notaba muy poco que hubiera algo en el punto
     exacto del carro (solo el rombo flotante). Esto dibuja un cajón de
     carga roto, a nivel de suelo, en las coordenadas reales del carro. */
  function drawCrate(c, x, y) {
    const p = U.worldToScreen(x, y, 20);
    c.save();
    c.translate(p.x, p.y);
    c.fillStyle = 'rgba(0,0,0,.28)';
    c.beginPath(); c.ellipse(0, 15, 19, 7, 0, 0, Math.PI * 2); c.fill();
    const w = 16, h = 20;
    c.fillStyle = '#5c4126';
    c.beginPath();
    c.moveTo(0, -h); c.lineTo(-w, -h * 0.55); c.lineTo(-w, h * 0.15); c.lineTo(0, h * 0.55);
    c.closePath(); c.fill();
    c.strokeStyle = '#241a10'; c.lineWidth = 1.6; c.stroke();
    c.fillStyle = '#7a5936';
    c.beginPath();
    c.moveTo(0, -h); c.lineTo(w, -h * 0.55); c.lineTo(w, h * 0.15); c.lineTo(0, h * 0.55);
    ...
    c.fillStyle = '#8f6a42';
    c.beginPath();
    c.moveTo(0, -h); c.lineTo(w, -h * 0.55); c.lineTo(0, -h * 0.1); c.lineTo(-w, -h * 0.55);
    c.closePath(); c.fill(); c.stroke();
    c.strokeStyle = 'rgba(0,0,0,.35)'; c.lineWidth = 1.2;
    c.beginPath(); c.moveTo(-w * 0.5, -h * 0.15); c.lineTo(w * 0.5, h * 0.05); c.stroke();
    c.restore();
  }
  ...
  U.draw = function () {
    ...
    if (state.stage === 1) {
      drawCrate(c, QUEST.wagon.x, QUEST.wagon.y);
      marker(c, QUEST.wagon.x, QUEST.wagon.y, '#d3a448', 'Carro');
    }
