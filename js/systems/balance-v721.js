(function (U) {
  ...
  /* FIX jugabilidad: el vendaje tenía una duración FIJA calculada a partir
     de Curar+Destreza (misma habilidad, mismo resultado siempre — de ahí
     que se sintiera "siempre 4.2s"). La regla pedida es que el tiempo sea
     aleatorio entre 1.5 y 4 s, con la destreza/curar inclinando la
     probabilidad hacia el extremo rápido sin volverla determinística. */
  U.healBandage = function () {
    ...
    // Sesga la probabilidad hacia el extremo rápido (1.5s) cuanto más alta
    // sea la maestría, pero sigue siendo un sorteo: nunca dos vendajes
    // iguales al mismo nivel de habilidad.
    const roll = Math.pow(Math.random(), 1 + masteryNorm * 2.2);
    const duration = 1.5 + roll * 2.5; // 1.5 a 4.0 s, con todos los decimales del medio
    ...
    // Curar es un cuidado corporal fino: entrena Destreza y Anatomía, igual
    // que golpear entrena Anatomía en combat.js.
