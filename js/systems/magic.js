(function (U) {
  U.spells = [
    ...
    { id: 'mend', name: 'Remiendo vital', icon: '✚', mana: 8, cd: 1, range: 0, cast: 0.4, level: 5, heal: 18 },
    ...
    { id: 'ward', name: 'Guarda arcana', icon: '◇', mana: 15, cd: 4, range: 0, cast: 0.8, level: 30, heal: 24 },
    ...
    { id: 'meteor', name: 'Fragmento celeste', icon: '☄️', mana: 28, cd: 3.5, range: 12, cast: 1.6, level: 80 },
  ...
  U.learnSpell = function (inventoryIndex) {
    const it = U.player.inventory[inventoryIndex], d = it && U.itemDefs[it.id];
    if (!d?.spell) return;
  ...
  U.finishCast = function (sp) {
    ...
    const colors = {
      ...
      curse: '#a85de2',
      heal: '#8fffc1',
      spark: '#ffe5a0', mend: '#9dffc3', ward: '#86c9ff', frost: '#7ccfff',
      meteor: '#ff984d', drain: '#a66be8', storm: '#c8a8ff',
    };
    for (let n = 0; n < 14; n++)
