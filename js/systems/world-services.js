    ...
    U.cities.forEach(c => {
      ...
      vendors.forEach(([catalog, name, dx, dy, color]) =>
        U.npcs.push({
          ...
          city: c.name, color, nameColor: '#69a9d8',
        }),
      );
      U.npcs.push(
        { type: 'banker', name: 'Banquero de ' + c.name, x: c.x - 4, y: c.y + 3, city: c.name, color: '#d9d2b2', nameColor: '#69a9d8' },
        { type: 'healer', name: 'Sanador de ' + c.name, x: c.x + 2, y: c.y - 6, city: c.name, color: '#ddd', nameColor: '#69a9d8' },
      );
      for (let i = 0; i < 4; i++) {
