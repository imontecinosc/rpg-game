(function(U){
  U.setupWorld=function(){
    U.cities.forEach(c=>{
      const vendors=[
        ['magic','Escriba arcano',3,1,'#755c9b'],['leather','Curtidor',4,3,'#855638'],
        ['iron','Armero',1,4,'#87919a'],['weapons','Maestro de armas',-2,4,'#9a7c55'],
        ['clothing','Sastre',-4,2,'#735c83'],['supplies','Proveedor',-4,-1,'#8b7655'],
        ['rare','Mercader de metales raros',3,-3,'#a482b9']
      ];
      ...
      U.npcs.push({type:'banker',name:'Banquero de '+c.name,x:c.x-2,y:c.y+1,city:c.name,color:'#d9d2b2',nameColor:'#69a9d8'},{type:'healer',name:'Sanador de '+c.name,x:c.x,y:c.y-2,city:c.name,color:'#ddd',nameColor:'#69a9d8'});
      for(let i=0;i<2;i++)U.npcs.push({type:'guard',name:'Guardia de '+c.name,x:c.x+U.rnd(-4,4),y:c.y+U.rnd(-4,4),city:c.name,color:'#8ea0b0',nameColor:'#69a9d8'});
    });
