(function (U) {
  ...
  ui.renderSpells = function () {
    ...
    const staff = U.player.equipment.weapon && U.itemDefs[U.player.equipment.weapon.id]?.visualModel === 'staff';
    const favorites = book ? U.player.favoriteSpells || [] : [];
    const buttons = [...document.querySelectorAll('.utility .util-btn')];
    const spells = U.spells.filter(sp => favorites.includes(sp.id) && (U.player.skills.Magia || 0) >= sp.level).slice(0, 6);
    const staffPassives = [
      {icon:'✦', name:'Canalización: el báculo escala con Magia e Inteligencia'},
      {icon:'◈', name:'Impacto arcano: posibilidad de daño mágico adicional'},
      {icon:'⌁', name:'Foco de roble: mejora la consistencia mágica'},
    ];
    buttons.forEach((button, index) => {
      const spell = spells[index];
      const passive = !spell && staff ? staffPassives[index] : null;
      button.classList.toggle('empty-ability', !spell && !passive);
      button.classList.toggle('passive-ability', !!passive);
      button.disabled = !!passive || (!spell && !passive);
      if (spell) button.dataset.spellId = spell.id;
      else delete button.dataset.spellId;
      button.dataset.tip = spell ? spell.name : passive ? passive.name : 'Ranura de habilidad vacía';
      button.textContent = spell ? spell.icon : passive ? passive.icon : '';
      button.onclick = null;
    });
    const strip = U.$('#spell-strip');
  ...
  ui.refreshInventory = function () {
    ...
    if (inv) {
      const activeTab = 'inventario';
      U.state.invTab = activeTab;
      const shown = U.player.inventory.map((it, i) => i);
      inv.innerHTML = '';
      // Fila de arriba (7 primeras casillas) enumerada 1-5, Q, R igual que la barra de equipables.
      const hotkeys = ['1', '2', '3', '4', '5', 'Q', 'R'];
      const totalCells = Math.max(27, shown.length);
      for (let cell = 0; cell < totalCells; cell++) {
        const invIdx = shown[cell],
          b = document.createElement('button');
        if (invIdx === undefined) {
          b.className = 'inventory-cell empty';
        } else {
          const it = U.player.inventory[invIdx],
            d = U.itemDefs[it.id],
            rareza = it.rarity || d.rarity || 'Común';
          b.className =
            'inventory-cell' +
            (d.type === 'weapon' && ['Épico', 'Legendario'].includes(rareza) ? ' weapon-glow' : '') +
            (U.state.selectedInventoryIndex === invIdx ? ' selected' : '');
          b.dataset.item = invIdx;
          b.dataset.rarity = rareza;
          if (d.visualColor) b.style.setProperty('--item-color', d.visualColor);
          b.title = `${d.name} — ${rareza}`;
          b.innerHTML = `<span>${d.icon}</span>${it.qty > 1 ? `<span class="count">${it.qty}</span>` : ''}`;
        }
        if (cell < hotkeys.length) {
          const k = document.createElement('span');
          k.className = 'cell-key';
          k.textContent = hotkeys[cell];
          b.appendChild(k);
        }
        inv.append(b);
      }
      U.$$('.inventory-tab').forEach(t => t.classList.toggle('active', t.dataset.tab === activeTab));
    }
    if (U.$('#inv-space'))
      U.$('#inv-space').textContent = `${U.player.inventory.length} / ${U.INV_SLOTS} ESPACIOS`;
    if (U.$('#inv-gold')) U.$('#inv-gold').textContent = `ORO: ${U.countItem('gold')}`;

    // Ranuras del maniquí: cada boton .pd-slot ya existe en el HTML fijo
    // (misma posicion del prototipo); solo se actualiza su icono y estado.
    ...
    U.equipmentSlots.forEach(sl => {
      const btn = U.$(`.pd-slot[data-slot="${sl}"]`);
      if (!btn) return;
      const meta = slotMeta[sl],
        it = U.player.equipment[sl],
        d = it && U.itemDefs[it.id],
        bloqueada = sl === 'shield' && escudoOcupado && !it,
        rareza = d ? it.rarity || d.rarity || 'Común' : '';
      btn.classList.toggle('selected', U.state.selectedEquipmentSlot === sl);
      btn.classList.toggle('bloqueada', bloqueada);
      btn.classList.toggle(
        'weapon-glow',
        !!(d?.type === 'weapon' && ['Épico', 'Legendario'].includes(rareza)),
      );
      if (d?.visualColor) btn.style.setProperty('--item-color', d.visualColor);
      const icon = btn.querySelector('.pd-icon');
      if (icon) icon.textContent = d ? d.icon : meta?.fa || '·';
      btn.title = d
        ? `${slotNames[sl]} — ${d.name}`
        : bloqueada
    ...
    if (U.$('#pd-stat-damage')) U.$('#pd-stat-damage').textContent = st.damage;
    if (U.$('#pd-stat-armor')) U.$('#pd-stat-armor').textContent = Math.round(st.armor || 0);
    if (U.$('#pd-stat-weight')) U.$('#pd-stat-weight').textContent = `${peso.toFixed(0)}/${U.player.maxWeight}`;
  ...
  ui.showItem = function (i) {
    ...
    U.$('#item-detail').classList.add('active');
  };
  ...
  ui.openSettings = () => {
    const muted = U.$('#sound-muted');
    const volume = U.$('#sound-volume');
    if (muted) muted.checked = U.audio.muted;
    if (volume) volume.value = Math.round(U.audio.volume * 100);
    const dialog = U.$('#settings-dialog');
  ...
  ui.bind = function () {
    ...
    const portraitSkill = U.$('#portrait-skills-btn');
    if (portraitSkill) {
      portraitSkill.onclick = ui.openSkills;
      portraitSkill.onkeydown = event => {
        if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); ui.openSkills(); }
      };
    }
