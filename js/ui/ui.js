(function (U) {
  ...
  ui.swatch = d =>
    `<i class="item-swatch" style="background:${d.rarityColor || materialColors[d.material] || '#777'}"></i>`;
  ...
  ui.renderSpells = function () {
    U.$('#spell-strip').innerHTML = U.spells
      .map(
        (s, i) =>
          `<button class="spell ${U.state.selectedSpell === s.id ? 'active' : ''}" data-spell="${i}">${s.icon}<small>${s.name}</small></button>`,
      )
      .join('');
  };
  ui.refreshInventory = function () {
    ...
    U.player.inventory.forEach((it, i) => {
      ...
      b.className = 'inv-slot' + (U.state.selectedInventoryIndex === i ? ' selected' : '');
      b.dataset.item = i;
    ...
    U.equipmentSlots.forEach(sl => {
      ...
      const titulo = d
        ...
        : bloqueada
          ...
          : `${slotNames[sl]} — vacío`;
      cols[meta.col] +=
        `<button class="${clase}${sel}" data-slot="${sl}" data-rarity="${rareza}" title="${titulo}" aria-label="${titulo}">${cuerpo}</button>`;
    });
  ...
  ui.showItem = function (i) {
    ...
    U.$('#item-detail').innerHTML =
      `<h3>${d.icon} ${d.name}${it.qty > 1 ? ` ×${it.qty}` : ''}</h3><div class="item-description">${ui.describeItem(it)}</div><div class="detail-actions">${d.slot ? `<button data-equip="${i}">Equipar</button>` : ''}${d.insurable ? `<button data-insure="${i}">${it.insured ? 'Quitar seguro' : 'Asegurar'}</button>` : ''}<button data-drop="${i}">Soltar</button></div>`;
  };
  ui.showEquipped = function (slot) {
    ...
    const d = U.itemDefs[it.id];
    U.$('#item-detail').innerHTML =
      `<h3>${d.icon} ${d.name}</h3><div class="item-description">${ui.describeItem(it)}</div><div class="detail-actions"><button data-unequip="${slot}">Desequipar</button>${d.insurable ? `<button data-eqinsure="${slot}">${it.insured ? 'Quitar seguro' : 'Asegurar'}</button>` : ''}</div>`;
  };
  ...
  ui.refreshCharacter = function () {
    ...
    U.$('#sheet-skill-cap').textContent = `${total.toFixed(1)} · sin cap`;
  ...
  ui.recoverPlayerCorpse = function (c) {
    ...
    ui.refreshAll();
    U.toast(
      recovered ? `Recuperaste ${recovered} objetos y tu equipo anterior.` : 'Tu cadáver estaba vacío.',
    );
  };
  ...
  ui.refreshCorpse = function () {
    ...
    grid.innerHTML =
      c.items
        .map((it, i) => {
          const d = U.itemDefs[it.id];
          return `<button class="loot-slot ${sel === i ? 'selected' : ''}" data-loot-select="${i}" title="${d.name}">${ui.swatch(d)}${d.icon}<small>${d.name}</small>${it.qty > 1 ? `<b>${it.qty}</b>` : ''}</button>`;
        })
  ...
  ui.bind = function () {
    ...
    document.addEventListener('click', e => {
      ...
      if (!t) return;
      if (t.dataset.spell !== undefined) return U.castSpell(+t.dataset.spell);
