(function (U) {
  ...
  ui.showEquipped = function (slot) {
    ...
    const spellbookControls =
      d.type === 'spellbook'
        ? `<div class="spellbook-list">${U.spells
            .filter(s => U.player.knownSpells.includes(s.id))
            .map(s => `<button data-favorite-spell="${s.id}" class="${U.player.favoriteSpells.includes(s.id) ? 'active' : ''}">${s.icon} ${s.name} · Magia ${s.level}</button>`)
            .join('') || '<p>El grimorio aún está vacío.</p>'}</div>`
        : '';
    U.$('#item-detail').innerHTML =
